...
async function Pt(i, e) {
  ...
  return n || i.classList.add("unstyled"), oe(t.theme), Ce(t) ? Dt(i, t, s) : Mt(i, t, s);
}
function Mt(i, e, t) {
  const n = At(e.layout, {
    container: i,
...
const ie = ["Image", "BufferA", "BufferB", "BufferC", "BufferD"], Ut = ["BufferA", "BufferB", "BufferC", "BufferD"], Pe = ["iChannel0", "iChannel1", "iChannel2", "iChannel3"], Bt = "default", Me = !0, It = "light", De = /* @__PURE__ */ new Set([
  "iResolution",
  "iTime",
  "iTimeDelta",
  "iFrame",
...
async function Qt(i, e, t, n, s) {
  ...
  const c = await Be(r, s), l = Ie(e, n), d = await Kt(l, r, {
    config: a,
    script: c,
...
const tn = /* @__PURE__ */ Object.assign({ "./notes/3d/04-sculpting/shaders/mug-in-scene/image.glsl": () => Promise.resolve().then(() => hn).then((i) => i.default) }), nn = /* @__PURE__ */ Object.assign({}), sn = /* @__PURE__ */ Object.assign({}), rn = /* @__PURE__ */ Object.assign({});
let ne = null;
async function on() {
  return ne || (ne = await Qt("notes/3d/04-sculpting/shaders/mug-in-scene", tn, nn, sn, rn)), ne;
}
      ...
      for (var C in f)
        if (!(!f.hasOwnProperty(C) || !f[C])) {
          ...
          for (var R = 0; R < v.length; ++R) {
            ...
            for (var le = F.pattern || F, L = p.next, M = g; L !== h.tail && !(x && M >= x.reach); M += L.value.length, L = L.next) {
              var I = L.value;
              ...
              if (!(I instanceof c)) {
                var X = 1, P;
                if (ce) {
                  if (P = l(le, M, m, B), !P || P.index >= m.length)
                    break;
                  var j = P.index, Xe = P.index + P[0].length, k = M;
                  for (k += L.value.length; j >= k; )
                    L = L.next, k += L.value.length;
                  if (k -= L.value.length, M = k, L.value instanceof c)
                    continue;
                  for (var N = L; N !== h.tail && (k < Xe || typeof N.value == "string"); N = N.next)
                    X++, k += N.value.length;
                  X--, I = m.slice(M, k), P.index -= M;
                } else if (P = l(le, 0, I, B), !P)
                  continue;
                var j = P.index, z = P[0], J = I.slice(0, j), ue = I.slice(j + z.length), Q = M + I.length;
                x && Q > x.reach && (x.reach = Q);
                var Y = L.prev;
                J && (Y = E(h, Y, J), M += J.length), y(h, Y, X);
                ...
                if (L = E(h, Y, je), ue && E(h, L, ue), X > 1) {
                  ...
                  u(m, h, f, L.prev, M, ee), x && ee.reach > x.reach && (x.reach = ee.reach);
                }
              }
            }
          }
