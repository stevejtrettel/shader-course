...
We start with the simplest case: one sphere, one light, analytical intersection. This establishes the machinery — rays, hits, lighting, camera control — that carries through the rest of the 3D rendering chapters.
...
Even with just one object, this establishes the pattern we'll use throughout: shapes carry their own data, including color.
...
A red disk. We found the sphere and used its color, but it looks flat — every hit pixel gets the same shade. To see the curvature, we need lighting, and lighting depends on surface orientation.
...
where $\mathbf{p}$ is the hit point. We can visualize normals as colors by mapping components from $[-1,1]$ to $[0,1]$:
...
Now we see the shape: red on the right ($+x$), green on top ($+y$), blue facing us ($+z$). The sphere is revealing its geometry through the normal vectors.


## Lighting

A flat-colored sphere looks like a disk. To see the curvature, we need lighting — surfaces facing the light should be bright, surfaces facing away should be dark.

The key insight: brightness depends on the angle between the surface normal and the light direction. When they align (surface faces the light directly), maximum brightness. When perpendicular or facing away, darkness. The dot product captures this:

$$\text{brightness} = \max(0, \mathbf{n} \cdot \mathbf{l})$$

where $\mathbf{n}$ is the normal and $\mathbf{l}$ points toward the light. The $\max$ prevents negative values for surfaces facing away.
...
Three lines. The light direction is baked in — we'll generalize this later. For now, it just works: pass in the normal and color, get back a lit color.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (t > 0.0) {
        ...
        vec3 ambient = sphere.color * 0.15;
        color = ambient + shade(normal, sphere.color);
    }
...
The sphere looks 3D: bright where it faces the light, dark on the opposite side. The ambient term prevents the dark side from going completely black.
...
Drag to orbit. The sphere stays at the origin; the camera circles it. This is our complete first raytracer: one sphere, one light, full mouse control.
...
**Checkpoint 3: Normal Variations**

We visualized normals with `normal * 0.5 + 0.5`. Try these variations:
- `abs(normal)` — what's different?
...
3. Keep track of which hit is closer

Hint: intersect both, then use the smaller positive `t`.
...
Implement this and give the ground a different color. Watch for division by zero when $d_y = 0$ (ray parallel to plane).
...
Start simple: just color the reflection ray's direction to verify it's correct, then add intersection.

**Challenge 3: Multiple Lights**

The `shade` function has one hardcoded light. Modify it to accept a light direction as a parameter, then call it twice with different directions:

```glsl
vec3 color = ambient;
color += shade(normal, surfaceColor, normalize(vec3(1, 1, 1)));
color += shade(normal, surfaceColor, normalize(vec3(-1, 0.5, 0)));
```

Try a warm key light and a cool fill light by multiplying each contribution by a color.
