...
{{< shader layout="tabbed" dielectric >}}
...
Compare this to the uncoupled version from §5. The ad hoc `vec3(0.3)` is replaced by Fresnel — a physically derived quantity that depends on the angle and the material. For dielectrics, `specColor` is `vec3(0.04)` — a dim, achromatic highlight that brightens at grazing angles. For metals, `specColor` is the albedo — a bright, colored highlight. The diffuse term carries the factor $(1 - F_i)$: energy that reflects at the interface cannot also scatter diffusely.
