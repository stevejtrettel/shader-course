...
Structure appears: tendrils, spirals, bulbs around the boundary. Points near the boundary take many iterations to escape (bright); points far away escape quickly (dark).
...
The inverted point lies on the same ray from the origin, but at reciprocal distance: if $\mathbf{p}$ is at distance $r$, its image is at distance $1/r$. Points inside the circle map outside; points outside map inside; points on the circle stay fixed.
...
The Apollonian gasket is a fractal circle packing. Start with four mutually tangent circles, three inside one, and fill each curved gap with a circle tangent to its three neighbors. Repeat.
