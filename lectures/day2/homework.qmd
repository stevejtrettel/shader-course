...
Create a grid where each cell shows the Julia set for a different parameter $c$. Use the grid system from Day 1:

```glsl
float L = 0.5;  // cell size in coordinate space
vec2 cell_id = floor(p / L + 0.5);       // which cell (centered at origin)
vec2 cell_center = cell_id * L;           // center of that cell
vec2 cell_p = p - cell_center;            // position within cell, in [-L/2, L/2]
```

Map `cell_id` to a parameter $c$ in an interesting region of the complex plane (try the window $[-2, 0.5] \times [-1.25, 1.25]$). Use `cell_p` scaled to $[-2, 2]$ as the starting point $z$ for the Julia iteration.
