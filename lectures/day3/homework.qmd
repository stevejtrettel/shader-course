...
The hyperbolic law of cosines gives the side length opposite angle $\pi/r$:
$$s_r = \text{arccosh}\left(\frac{\cos(\pi/r) + \cos(\pi/p)\cos(\pi/q)}{\sin(\pi/p)\sin(\pi/q)}\right)$$

From here, we can find the Euclidean height of the third vertex:
```glsl
float sr = acosh((cos(PI/r) + cos(PI/p)*cos(PI/q)) / (sin(PI/p)*sin(PI/q)));
float eucH = sin(PI/p) * exp(sr);
```

And the center and radius of the third geodesic:
```glsl
float want = eucH / tan(PI/q);
float center = -cos(PI/p) - want;
float radius = sqrt(want*want + eucH*eucH);
```

**Exercise:** Work through this calculation by hand for $(2,3,7)$ and verify you get `center ≈ -0.7665` and `radius ≈ 1.533`.

### Part C: Implementation

Write a function that takes `p`, `q`, `r` as parameters and returns the three half-spaces. Then build the tiling with adjustable parameters (use constants or uniforms).

Validate that your formula reproduces the known values:
- $(2,3,7)$: center $\approx -0.7665$, radius $\approx 1.533$
- $(2,4,5)$: center $\approx -1.70$, radius $\approx 2.40$
- $(2,3,\infty)$: should give a vertical line at $x = 0.5$ (what happens to the formula as $r \to \infty$?)
...
Generalize your implementation to handle arbitrary $(p,q,r)$ triangles. Test with $(3,3,4)$: all three edges are semicircles, and the triangle has no right angles.