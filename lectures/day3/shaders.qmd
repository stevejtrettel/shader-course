...
This shader introduces the **ternary operator**: `condition ? a : b` returns `a` if the condition is true, `b` if false. It's a compact alternative to if/else for simple assignments:

```glsl
// These are equivalent:
vec3 color = (parity < 0.5) ? light : dark;
...
You'll see this pattern constantly in shader code. We'll use it throughout the rest of the course.
:::
