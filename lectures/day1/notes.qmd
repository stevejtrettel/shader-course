...
A family of elliptic curves $y^2 = x^3 + ax + b$, drawn for several values of $a$ at once, with $b$ varying across the screen. Watch carefully and you'll see singularities appear and disappear along the discriminant locus $4a^3 + 27b^2 = 0$. The whole thing runs in real time, recomputed sixty times per second.

The key idea: to draw a curve $F(x,y) = 0$, color a pixel yellow if $|F|$ is small, and blue otherwise. The entire machinery of implicit curves becomes accessible in a few lines of code.
...
We now have a coordinate system: each pixel knows its location `p` in the plane, centered at the origin, aspect-corrected, scaled to a reasonable range. The next question is what to do with it.

The answer, throughout this course, will almost always be: compute a distance. Distances tell us how far we are from shapes, curves, boundaries. A filled circle is "the set of points within distance $r$ of the origin." A curve is "the set of points at distance zero from the curve." Even when the geometry gets complicated, the pattern stays the same: compute a distance, then decide what color based on whether it's large or small.
