...
With proper coordinates, left vs right is just `p.x < 0.0`.

Much cleaner—and now we can draw shapes.
...
`iTime` = seconds since shader started. `sin(iTime)` oscillates $-1$ to $1$.
...
To use it as a coordinate, normalize the same way as `fragCoord`. Since we'll do this twice, let's write a helper function:
...
The sun is at the mouse position. The earth orbits around it:

```glsl
vec2 sun = normalize_coord(iMouse.xy);
vec2 earth = sun + 0.8 * vec2(cos(iTime), sin(iTime));
```

##

{{< shader-slide day1/sun-earth >}}
...
- Animation: `iTime` for motion, `iMouse` for interaction
- Implicit curves: $F(x,y) = 0$ with gradient correction
- Parameter exploration: mouse controls curve families