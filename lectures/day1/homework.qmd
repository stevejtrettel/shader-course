...
```glsl
vec2 cell_id = floor(p / L);              // which cell (integers)
vec2 cell_center = (cell_id + 0.5) * L;   // world position of cell center
vec2 local = p - cell_center;             // offset from center
```

**How it works:**

- `floor(p / L)` asks "how many whole cells from the origin?"—giving integer indices $(i, j)$
- Cell $i$ spans from $iL$ to $(i+1)L$, so its center is at $(i + 0.5)L$
- Subtracting the center gives local coordinates in the range $[-L/2, L/2]$

That's it. The geometry is transparent: find the cell, find its center, measure from there.
...
vec2 cell_id = floor(p / L);                // integer cell index
vec2 cell_center = (cell_id + 0.5) * L;     // center of this cell
