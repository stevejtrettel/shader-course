...
**Part D: GRIN Lens.** A gradient-index lens has smoothly varying speed:
```glsl
float n = 1.0 + 0.5 * exp(-d * d / 5000.0);  // higher index at center
float c = 1.0 / n;
```

This creates focusing without any sharp boundary. Real GRIN lenses are used in fiber optics and compact cameras.

**Extensions:**
- Add absorption (multiply $u$ by a decay factor in the glass region)
- Create a prism (triangular glass region) and watch dispersion... wait, our waves don't have wavelength-dependent speed. Can you fake it?
