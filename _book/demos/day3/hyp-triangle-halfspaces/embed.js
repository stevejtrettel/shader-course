      ...
      for (var y in u)
        if (!(!u.hasOwnProperty(y) || !u[y])) {
          ...
          for (var w = 0; w < h.length; ++w) {
            if (m && m.cause == y + "," + w)
              return;
            var C = h[w], S = C.inside, P = !!C.lookbehind, $ = !!C.greedy, X = C.alias;
            if ($ && !C.pattern.global) {
            ...
            for (var W = C.pattern || C, L = l.next, B = d; L !== a.tail && !(m && B >= m.reach); B += L.value.length, L = L.next) {
              ...
              if (!(U instanceof E)) {
                ...
                var z = L.prev;
                G && (z = T(a, z, G), B += G.length), _(a, z, I);
                var ae = new E(y, S ? c.tokenize(N, S) : N, X, N);
                if (L = T(a, z, ae), K && T(a, L, K), I > 1) {
                  var q = {
                    cause: y + "," + w,
                    reach: V
                  };
...
const Fe = "course/day3/hyp-triangle-halfspaces";
async function Re() {
  return _e(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/hyp-triangle-halfspaces/image.glsl": () => Promise.resolve().then(() => Ce).then((r) => r.default)
