  ...
  initRuntimePasses() {
    ...
    for (const o of s) {
      ...
      try {
        ...
        }, b = j(e, this._width, this._height), g = j(e, this._width, this._height), T = Z(e, b), _ = {
          name: o,
...
class ge {
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((v) => v.isFromCommon), r = e.filter((v) => !v.isFromCommon), c = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: v, error: b, source: g, isFromCommon: T, originalLine: _ }) => {
      const C = b.replace(`Shader compilation failed:
`, "");
      let x = C;
      return T && _ !== null && (x = C.replace(/Line \d+:/, `Line ${_}:`), x = x.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${_}:`)), {
...
`), f = 3, c = Math.max(0, s - f - 1), E = Math.min(o.length, s + f);
    return o.slice(c, E).map((g, T) => {
      const _ = c + T + 1, C = _ === s, x = String(_).padStart(4, " "), F = this.escapeHTML(g);
...
`), s = 3, o = Math.max(0, e - s - 1), f = Math.min(r.length, e + s);
    return r.slice(o, f).map((v, b) => {
      const g = o + b + 1, T = g === e, _ = String(g).padStart(4, " "), C = this.escapeHTML(v);
      return T ? `<span class="error-line-highlight">${_} │ ${C}</span>` : `<span class="context-line">${_} │ ${C}</span>`;
    }).join("");
  }
  /**
          ...
          for (var w = 0; w < h.length; ++w) {
            ...
            for (var W = A.pattern || A, L = l.next, B = d; L !== a.tail && !(m && B >= m.reach); B += L.value.length, L = L.next) {
              ...
              if (!(U instanceof E)) {
                ...
                var N = L.prev;
                G && (N = T(a, N, G), B += G.length), _(a, N, $);
                var ae = new E(y, S ? c.tokenize(z, S) : z, X, z);
...
class be {
  ...
  buildCodePanel() {
    ...
    `, b = (g) => {
      ...
      C.className = "language-cpp", C.textContent = T.source, _.appendChild(C), o.innerHTML = "", o.appendChild(_), te.highlightElement(C);
    };
    f.addEventListener("click", async () => {
      ...
      _.className = "tab-button", _.textContent = g.name, T === 0 && _.classList.add("active"), _.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((C) => C.classList.remove("active")), _.classList.add("active"), b(T);
      }), s.appendChild(_);
...
`), T = document.createElement("pre"), _ = document.createElement("div");
        _.className = "tabbed-line-numbers", _.innerHTML = g.map((F, R) => R + 1).join(`
`);
        const C = document.createElement("div");
        C.className = "tabbed-code-content";
        const x = document.createElement("code");
        x.className = "language-cpp", x.textContent = o, C.appendChild(x), T.appendChild(_), T.appendChild(C), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(T), te.highlightElement(x);
      }
    };
...
async function we(n, e, t, r) {
  ...
  const T = ((d = o.meta) == null ? void 0 : d.title) || n.split("-").map((h) => h.charAt(0).toUpperCase() + h.slice(1)).join(" "), _ = ((m = o.meta) == null ? void 0 : m.author) || null, C = ((y = o.meta) == null ? void 0 : y.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
    meta: { title: T, author: _, description: C },
    layout: x,
    controls: F,
    commonSource: f,
    passes: g,
