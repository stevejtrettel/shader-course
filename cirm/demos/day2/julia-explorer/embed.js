...
class Ee {
  constructor(e) {
    ...
    this.gl = e.gl, this.project = e.project, this._width = this.gl.drawingBufferWidth, this._height = this.gl.drawingBufferHeight, this.initExtensions(), this._blackTexture = he(this.gl);
    const t = de(this.gl);
  ...
  initRuntimePasses() {
    ...
    for (const c of s) {
      ...
      try {
        ...
        }, T = N(e, this._width, this._height), E = N(e, this._width, this._height), x = Y(e, T), y = {
          name: c,
...
`), p = 3, o = Math.max(0, s - p - 1), m = Math.min(c.length, s + p);
    return c.slice(o, m).map((E, x) => {
      const y = o + x + 1, R = y === s, _ = String(y).padStart(4, " "), w = this.escapeHTML(E);
  ...
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, c = 0, p = {}, o = {
      ...
      tokenize: function(i, a) {
        ...
        var h = new E();
        return x(h, h.head, i), T(i, h, a, h.head, 0), R(h);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var v in u)
        if (!(!u.hasOwnProperty(v) || !u[v])) {
          ...
          for (var F = 0; F < f.length; ++F) {
            if (d && d.cause == v + "," + F)
              return;
            var C = f[F], M = C.inside, D = !!C.lookbehind, V = !!C.greedy, re = C.alias;
            if (V && !C.pattern.global) {
              var ae = C.pattern.toString().match(/[imsuy]*$/)[0];
              C.pattern = RegExp(C.pattern.source, ae + "g");
            }
            for (var q = C.pattern || C, S = l.next, B = h; S !== a.tail && !(d && B >= d.reach); B += S.value.length, S = S.next) {
              ...
              if (!(P instanceof m)) {
                ...
                if (V) {
                  ...
                  continue;
                var I = L.index, j = L[0], H = P.slice(0, I), W = P.slice(I + j.length), X = B + P.length;
                d && X > d.reach && (d.reach = X);
                var O = S.prev;
                H && (O = x(a, O, H), B += H.length), y(a, O, $);
                var se = new m(v, M ? o.tokenize(j, M) : j, re, j);
  ...
  buildCodePanel() {
    ...
    p.addEventListener("click", async () => {
      ...
      y.className = "tab-button", y.textContent = E.name, x === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((R) => R.classList.remove("active")), y.classList.add("active"), T(x);
      }), s.appendChild(y);
...
const Re = "course/day2/julia-explorer";
async function Ae() {
  return we(Re, /* @__PURE__ */ Object.assign({
    "/demos/course/day2/julia-explorer/image.glsl": () => Promise.resolve().then(() => Se).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day2/julia-explorer/config.json": () => Promise.resolve().then(() => Me).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function ke(n) {
  ...
  const t = await Ae(), r = ye(t.layout, {
    container: e,
    project: t
  }), s = new be({
...
const Ce = `vec2 cmul(vec2 z, vec2 w) {
