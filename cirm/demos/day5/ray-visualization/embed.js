...
const Be = "course/day5/ray-visualization";
async function Le() {
  return Ae(Be, /* @__PURE__ */ Object.assign({
    "/demos/course/day5/ray-visualization/image.glsl": () => Promise.resolve().then(() => Ue).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day5/ray-visualization/config.json": () => Promise.resolve().then(() => $e).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Oe(n) {
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var F = 0; F < b.length; ++F) {
            if (m && m.cause == _ + "," + F)
              return;
            var A = b[F], k = A.inside, V = !!A.lookbehind, K = !!A.greedy, ce = A.alias;
            if (K && !A.pattern.global) {
              var le = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, le + "g");
            }
            for (var q = A.pattern || A, S = d.next, L = p; S !== o.tail && !(m && L >= m.reach); L += S.value.length, S = S.next) {
              var P = S.value;
