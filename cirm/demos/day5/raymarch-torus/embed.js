...
const Be = "course/day5/raymarch-torus";
async function Le() {
  return Se(Be, /* @__PURE__ */ Object.assign({
    "/demos/course/day5/raymarch-torus/image.glsl": () => Promise.resolve().then(() => Ue).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day5/raymarch-torus/config.json": () => Promise.resolve().then(() => $e).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Oe(r) {
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var F = 0; F < b.length; ++F) {
            if (m && m.cause == _ + "," + F)
              return;
            var S = b[F], k = S.inside, V = !!S.lookbehind, q = !!S.greedy, ce = S.alias;
            if (q && !S.pattern.global) {
              var le = S.pattern.toString().match(/[imsuy]*$/)[0];
              S.pattern = RegExp(S.pattern.source, le + "g");
            }
            for (var K = S.pattern || S, A = d.next, L = p; A !== o.tail && !(m && L >= m.reach); L += A.value.length, A = A.next) {
              var P = A.value;
