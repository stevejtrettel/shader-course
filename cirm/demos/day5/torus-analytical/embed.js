...
function be(r, e, t, n) {
  ...
  for (let a = 0; a < 256; a++) {
    const f = t.get(a) || !1, g = n.get(a) || 0, y = (0 * 256 + a) * 4;
    i[y + 0] = f ? 1 : 0, i[y + 1] = f ? 1 : 0, i[y + 2] = f ? 1 : 0, i[y + 3] = 1;
    const w = (2 * 256 + a) * 4;
...
class ve {
  ...
  step(e, t) {
    ...
    const y = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const w of y) {
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      n.deleteTexture(s.currentTexture), n.deleteTexture(s.previousTexture), n.deleteFramebuffer(s.framebuffer), s.currentTexture = O(n, e, t), s.previousTexture = O(n, e, t), s.framebuffer = Z(n, s.currentTexture);
  }
  /**
  ...
  initRuntimePasses() {
    ...
    for (const l of s) {
      ...
      try {
        ...
        }, y = O(e, this._width, this._height), w = O(e, this._width, this._height), x = Z(e, y), C = {
          name: l,
        ...
        const g = f instanceof Error ? f.message : String(f), y = this.getLineMapping(), w = g.match(/ERROR:\s*\d+:(\d+):/);
        let x = !1, C = null;
        if (w && this.project.commonSource) {
          const R = parseInt(w[1], 10), v = y.boilerplateLinesBeforeCommon + 2, E = v + y.commonLineCount - 1;
          R >= v && R <= E && (x = !0, C = R - v + 1);
        }
   ...
   */
  getLineMapping() {
    const e = ye + 1, t = this.project.commonSource ? this.project.commonSource.split(`
`).length : 0;
...
class Te {
  constructor(e) {
    ...
    this.gl = t, this.updateCanvasSize(), this.engine = new ve({
      gl: this.gl,
      project: e.project
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((g) => g.isFromCommon), n = e.filter((g) => !g.isFromCommon), a = [...t.length > 0 ? [t[0]] : [], ...n].map(({ passName: g, error: y, source: w, isFromCommon: x, originalLine: C }) => {
      const R = y.replace(`Shader compilation failed:
`, "");
      let v = R;
      return x && C !== null && (v = R.replace(/Line \d+:/, `Line ${C}:`), v = v.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${C}:`)), {
        passName: x ? "common.glsl" : g,
        error: this.parseShaderError(v),
        codeContext: x ? this.extractCodeContextFromCommon(C) : this.extractCodeContext(v, w)
      };
    }).map(({ passName: g, error: y, codeContext: w }) => `
      <div class="error-section">
        <div class="error-pass-name">${g}</div>
        <pre class="error-content">${this.escapeHTML(y)}</pre>
...
`), i = 3, a = Math.max(0, s - i - 1), f = Math.min(l.length, s + i);
    return l.slice(a, f).map((w, x) => {
      const C = a + x + 1, R = C === s, v = String(C).padStart(4, " "), E = this.escapeHTML(w);
      return R ? `<span class="error-line-highlight">${v} │ ${E}</span>` : `<span class="context-line">${v} │ ${E}</span>`;
    }).join("");
  }
  /**
...
`), s = 3, l = Math.max(0, e - s - 1), i = Math.min(n.length, e + s);
    return n.slice(l, i).map((g, y) => {
      const w = l + y + 1, x = w === e, C = String(w).padStart(4, " "), R = this.escapeHTML(g);
    ...
    const l = async (i) => {
      ...
      if (this.activeTabIndex = i, t.querySelectorAll(".tabbed-tab-button").forEach((f, g) => {
        ...
        try {
          ...
          this.editorInstance = g(this.editorContainer, f, (y) => {
            this.modifiedSources.set(a.passName, y);
          });
        } catch (g) {
          console.error("Failed to load editor:", g);
          const y = document.createElement("textarea");
          y.className = "tabbed-fallback-textarea", y.value = f, y.addEventListener("input", () => {
            this.modifiedSources.set(a.passName, y.value);
          }), this.editorContainer.appendChild(y);
        }
      } else {
...
async function Ae(r, e, t, n) {
  ...
  if (s in t) {
    const i = await t[s]();
    return i.Image || i.BufferA || i.BufferB || i.BufferC || i.BufferD ? Se(r, i, e, n) : ee(r, e, i);
  } else
    return ee(r, e);
}
...
async function Se(r, e, t, n) {
  ...
  const w = e.title || r.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" "), x = e.author || null, C = e.description || null, R = e.layout || "tabbed", v = e.controls ?? !0;
  return {
    root: `/demos/${r}`,
...
const ke = "course/day5/torus-analytical";
async function Be() {
  return Ae(ke, /* @__PURE__ */ Object.assign({
    "/demos/course/day5/torus-analytical/image.glsl": () => Promise.resolve().then(() => Ue).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day5/torus-analytical/config.json": () => Promise.resolve().then(() => $e).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Oe(r) {
  ...
  const t = await Be(), n = Fe(t.layout, {
    container: e,
    project: t
  }), s = new Te({
...
(function(r) {
  ...
  var t = function(n) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, l = 0, i = {}, a = {
      ...
      languages: {
        ...
        insertBefore: function(c, o, u, d) {
          ...
          var F = d[c];
          return d[c] = m, a.languages.DFS(a.languages, function(A, L) {
            L === F && A != c && (this[A] = m);
          }), m;
        },
      ...
      highlightElement: function(c, o, u) {
        ...
        function F(L) {
          b.highlightedCode = L, a.hooks.run("before-insert", b), b.element.innerHTML = b.highlightedCode, a.hooks.run("after-highlight", b), a.hooks.run("complete", b), u && u.call(b.element);
        }
      ...
      tokenize: function(c, o) {
        ...
        var p = new w();
        return x(p, p.head, c), y(c, p, o, p.head, 0), R(p);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var F = 0; F < b.length; ++F) {
            if (m && m.cause == _ + "," + F)
              return;
            var A = b[F], L = A.inside, V = !!A.lookbehind, q = !!A.greedy, ce = A.alias;
            if (q && !A.pattern.global) {
              var le = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, le + "g");
            }
            for (var K = A.pattern || A, S = d.next, B = p; S !== o.tail && !(m && B >= m.reach); B += S.value.length, S = S.next) {
              var P = S.value;
              ...
              if (!(P instanceof f)) {
                var U = 1, k;
                if (q) {
                  if (k = g(K, B, c, V), !k || k.index >= c.length)
                    break;
                  var N = k.index, ue = k.index + k[0].length, M = B;
                  for (M += S.value.length; N >= M; )
                    S = S.next, M += S.value.length;
                  if (M -= S.value.length, B = M, S.value instanceof f)
                    continue;
                  for (var D = S; D !== o.tail && (M < ue || typeof D.value == "string"); D = D.next)
                    U++, M += D.value.length;
                  U--, P = c.slice(B, M), k.index -= B;
                } else if (k = g(K, 0, P, V), !k)
                  continue;
                var N = k.index, $ = k[0], z = P.slice(0, N), W = P.slice(N + $.length), X = B + P.length;
                m && X > m.reach && (m.reach = X);
                var j = S.prev;
                z && (j = x(o, j, z), B += z.length), C(o, j, U);
                var de = new f(_, L ? a.tokenize($, L) : $, ce, $);
                if (S = x(o, j, de), W && x(o, S, W), U > 1) {
                  var G = {
                    cause: _ + "," + F,
                    reach: X
                  };
                  y(c, o, u, S.prev, B, G), m && G.reach > m.reach && (m.reach = G.reach);
                }
              }
            }
          }
    ...
    if (!a.manual) {
      ...
      T === "loading" || T === "interactive" && v && v.defer ? document.addEventListener("DOMContentLoaded", E) : window.requestAnimationFrame ? window.requestAnimationFrame(E) : window.setTimeout(E, 16);
    }
    return a;
  }(e);
  r.exports && (r.exports = t), typeof te < "u" && (te.Prism = t), t.languages.markup = {
    ...
    var n = "Loading…", s = function(v, E) {
      return "✖ Error " + v + " while fetching file: " + E;
      ...
      h: "c",
      tex: "latex"
    }, a = "data-src-status", f = "loading", g = "loaded", y = "failed", w = "pre[data-src]:not([" + a + '="' + g + '"]):not([' + a + '="' + f + '"])';
    function x(v, E, T) {
      var c = new XMLHttpRequest();
      c.open("GET", v, !0), c.onreadystatechange = function() {
    ...
    t.hooks.add("before-highlightall", function(v) {
      ...
      if (E.matches(w)) {
        v.code = "", E.setAttribute(a, f);
        var T = E.appendChild(document.createElement("CODE"));
        T.textContent = n;
        var c = E.getAttribute("data-src"), o = v.language;
        if (o === "none") {
...
`), E.hasAttribute("data-start") || E.setAttribute("data-start", String(b + 1));
          ...
          function(p) {
            E.setAttribute(a, y), T.textContent = p;
          }
        );
      }
...
`, Pe.highlightElement(f);
    const C = x.split(`
`);
    s.innerHTML = C.map((R, v) => `<span>${v + 1}</span>`).join(""), t && t(x);
  }
  function y() {
  ...
  function w(x) {
    if (x.key === "Tab") {
      x.preventDefault();
      const C = i.selectionStart, R = i.selectionEnd, v = i.value;
      i.value = v.substring(0, C) + "  " + v.substring(R), i.selectionStart = i.selectionEnd = C + 2, g();
    }
  }
  return i.addEventListener("input", g), i.addEventListener("scroll", y), i.addEventListener("keydown", w), g(), {
    getSource: () => i.value,
    setSource: (x) => {
      i.value = x, g();
    },
    destroy: () => {
      i.removeEventListener("input", g), i.removeEventListener("scroll", y), i.removeEventListener("keydown", w), r.removeChild(n);
    }
  };
}
