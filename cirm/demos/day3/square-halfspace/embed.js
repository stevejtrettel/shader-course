...
`), p = 3, o = Math.max(0, i - p - 1), m = Math.min(c.length, i + p);
    return c.slice(o, m).map((E, x) => {
      const _ = o + x + 1, C = _ === i, y = String(_).padStart(4, " "), w = this.escapeHTML(E);
            ...
            for (var q = A.pattern || A, S = l.next, B = h; S !== a.tail && !(d && B >= d.reach); B += S.value.length, S = S.next) {
              ...
              if (!(P instanceof m)) {
                ...
                if (V) {
                  ...
                  continue;
                var $ = L.index, N = L[0], z = P.slice(0, $), W = P.slice($ + N.length), X = B + P.length;
                d && X > d.reach && (d.reach = X);
                var O = S.prev;
                z && (O = x(a, O, z), B += z.length), _(a, O, I);
  ...
  buildCodePanel() {
    ...
    p.addEventListener("click", async () => {
      ...
      _.className = "tab-button", _.textContent = E.name, x === 0 && _.classList.add("active"), _.addEventListener("click", () => {
        i.querySelectorAll(".tab-button").forEach((C) => C.classList.remove("active")), _.classList.add("active"), T(x);
      }), i.appendChild(_);
...
async function we(n, e, t, r) {
  ...
  for (const d of p) {
    ...
    const M = await t[A](), D = [
      ...
      H((R = v.channels) == null ? void 0 : R.iChannel1, m),
