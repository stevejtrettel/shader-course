...
var g = (n, e, t) => ce(n, typeof e != "symbol" ? e + "" : e, t);
function K(n, e, t) {
...
function ue(n, e, t) {
  const r = K(n, n.VERTEX_SHADER, e), s = K(n, n.FRAGMENT_SHADER, t), c = n.createProgram();
  if (!c)
    throw new Error("Failed to create program object");
  if (n.attachShader(c, r), n.attachShader(c, s), n.linkProgram(c), !n.getProgramParameter(c, n.LINK_STATUS)) {
    const o = n.getProgramInfoLog(c);
    throw n.deleteProgram(c), n.deleteShader(r), n.deleteShader(s), new Error(`Program linking failed:
${o}`);
  }
  return n.detachShader(c, r), n.detachShader(c, s), n.deleteShader(r), n.deleteShader(s), c;
}
function le(n) {
...
function Y(n, e) {
  ...
  if (r !== n.FRAMEBUFFER_COMPLETE)
    throw n.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${fe(n, r)}`);
...
function pe(n, e, t, r) {
  ...
  for (let o = 0; o < 256; o++) {
    const m = t.get(o) || !1, v = r.get(o) || 0, T = (0 * 256 + o) * 4;
    p[T + 0] = m ? 1 : 0, p[T + 1] = m ? 1 : 0, p[T + 2] = m ? 1 : 0, p[T + 3] = 1;
    const E = (2 * 256 + o) * 4;
    p[E + 0] = v, p[E + 1] = v, p[E + 2] = v, p[E + 3] = 1;
  }
...
class Ee {
  constructor(e) {
    ...
    this.gl = e.gl, this.project = e.project, this._width = this.gl.drawingBufferWidth, this._height = this.gl.drawingBufferHeight, this.initExtensions(), this._blackTexture = de(this.gl);
    const t = he(this.gl);
  ...
  step(e, t) {
    ...
    const c = [this._width, this._height, 1], p = this._time, o = s, m = this._frame, v = t;
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      r.deleteTexture(s.currentTexture), r.deleteTexture(s.previousTexture), r.deleteFramebuffer(s.framebuffer), s.currentTexture = N(r, e, t), s.previousTexture = N(r, e, t), s.framebuffer = Y(r, s.currentTexture);
  }
  /**
  ...
  initProjectTextures() {
    ...
    for (const t of this.project.textures) {
      ...
      c.crossOrigin = "anonymous", c.onload = () => {
        ...
        e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, o), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, o), t.filter === "linear" && e.generateMipmap(e.TEXTURE_2D), s.width = c.width, s.height = c.height, console.log(`Loaded texture '${t.name}': ${c.width}x${c.height}`);
      }, c.onerror = () => {
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, r = le(e), s = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const c of s) {
      const p = t.passes[c];
      if (!p) continue;
      ...
      try {
        ...
        }, T = N(e, this._width, this._height), E = N(e, this._width, this._height), x = Y(e, T), _ = {
          name: c,
        ...
        const v = m instanceof Error ? m.message : String(m), T = this.getLineMapping(), E = v.match(/ERROR:\s*\d+:(\d+):/);
        let x = !1, _ = null;
        if (E && this.project.commonSource) {
          const C = parseInt(E[1], 10), y = T.boilerplateLinesBeforeCommon + 2, w = y + T.commonLineCount - 1;
          C >= y && C <= w && (x = !0, _ = C - y + 1);
        }
        this._compilationErrors.push({
          ...
          isFromCommon: x,
          originalLine: _
        }), console.error(`Failed to compile ${c}:`, v);
      }
    }
  }
  /**
   ...
   */
  getLineMapping() {
    const e = ge + 1, t = this.project.commonSource ? this.project.commonSource.split(`
`).length : 0;
...
`);
  ...
  preprocessCubemapTextures(e) {
    ...
    return e.replace(t, (r, s, c) => c.includes("fragCoord") || // Using fragCoord directly
    c.includes("/") || // Division (likely uv calculation)
    /\.xy\s*$/.test(c.trim()) || // Ends with .xy swizzle
    /\.st\s*$/.test(c.trim()) || // Ends with .st swizzle
    /^vec2\s*\(/.test(c.trim()) ? r : `texture(${s}, _st_dirToEquirect(${c}))`);
  }
  // ===========================================================================
  // Pass Execution
  // ===========================================================================
  ...
  bindChannelTextures(e) {
    const t = this.gl;
    for (let r = 0; r < 4; r++) {
      const s = e.projectChannels[r], c = this.resolveChannelTexture(s, e.name);
  ...
  resolveChannelTexture(e, t) {
    switch (e.kind) {
      ...
      case "texture2D": {
        const s = this._textures.find((c) => c.name === e.name);
        if (!s)
...
class ve {
  constructor(e) {
    ...
    this.gl = t, this.updateCanvasSize(), this.engine = new Ee({
      gl: this.gl,
      project: e.project
  ...
  setupMouseTracking() {
    const e = (r) => {
      const s = this.canvas.getBoundingClientRect(), c = (r.clientX - s.left) * this.pixelRatio, p = (s.height - (r.clientY - s.top)) * this.pixelRatio;
  ...
  screenshot() {
    ...
    this.canvas.toBlob((c) => {
      ...
      o.href = p, o.download = s, o.click(), URL.revokeObjectURL(p), console.log(`Screenshot saved: ${s}`);
    }, "image/png");
  }
  /**
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((v) => v.isFromCommon), r = e.filter((v) => !v.isFromCommon), o = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: v, error: T, source: E, isFromCommon: x, originalLine: _ }) => {
      const C = T.replace(`Shader compilation failed:
`, "");
      let y = C;
      return x && _ !== null && (y = C.replace(/Line \d+:/, `Line ${_}:`), y = y.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${_}:`)), {
        passName: x ? "common.glsl" : v,
        error: this.parseShaderError(y),
        codeContext: x ? this.extractCodeContextFromCommon(_) : this.extractCodeContext(y, E)
      };
    }).map(({ passName: v, error: T, codeContext: E }) => `
      <div class="error-section">
        <div class="error-pass-name">${v}</div>
...
`);
  ...
  extractCodeContext(e, t) {
    ...
    if (!r) return null;
    const s = parseInt(r[1], 10), c = t.split(`
`), p = 3, o = Math.max(0, s - p - 1), m = Math.min(c.length, s + p);
    return c.slice(o, m).map((E, x) => {
      const _ = o + x + 1, C = _ === s, y = String(_).padStart(4, " "), w = this.escapeHTML(E);
      return C ? `<span class="error-line-highlight">${y} │ ${w}</span>` : `<span class="context-line">${y} │ ${w}</span>`;
    }).join("");
  }
  /**
  ...
  extractCodeContextFromCommon(e) {
    ...
    if (!t) return null;
    const r = t.split(`
`), s = 3, c = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(c, p).map((v, T) => {
      const E = c + T + 1, x = E === e, _ = String(E).padStart(4, " "), C = this.escapeHTML(v);
      return x ? `<span class="error-line-highlight">${_} │ ${C}</span>` : `<span class="context-line">${_} │ ${C}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
   ...
   */
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, c = 0, p = {}, o = {
      /**
      ...
      util: {
        encode: function i(a) {
          return a instanceof m ? new m(a.type, i(a.content), a.alias) : Array.isArray(a) ? a.map(i) : a.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/\u00a0/g, " ");
        },
        /**
        ...
        clone: function i(a, u) {
          ...
          switch (o.util.type(a)) {
            case "Object":
              if (d = o.util.objId(a), u[d])
                return u[d];
              l = /** @type {Record<string, any>} */
              {}, u[d] = l;
              for (var h in a)
                a.hasOwnProperty(h) && (l[h] = i(a[h], u));
              return (
                /** @type {any} */
                l
              );
            case "Array":
              return d = o.util.objId(a), u[d] ? u[d] : (l = [], u[d] = l, /** @type {Array} */
              /** @type {any} */
        ...
        insertBefore: function(i, a, u, l) {
          ...
          for (var b in d)
            if (d.hasOwnProperty(b)) {
              ...
              u.hasOwnProperty(b) || (h[b] = d[b]);
            }
          var F = l[i];
          return l[i] = h, o.languages.DFS(o.languages, function(A, B) {
            B === F && A != i && (this[A] = h);
          }), h;
        },
        ...
        DFS: function i(a, u, l, d) {
          ...
          for (var b in a)
            if (a.hasOwnProperty(b)) {
              u.call(a, b, a[b], l || b);
              var f = a[b], F = o.util.type(f);
              F === "Object" && !d[h(f)] ? (d[h(f)] = !0, i(f, u, null, d)) : F === "Array" && !d[h(f)] && (d[h(f)] = !0, i(f, u, b, d));
            }
        }
      },
      plugins: {},
      ...
      highlightAllUnder: function(i, a, u) {
        ...
        for (var d = 0, h; h = l.elements[d++]; )
          o.highlightElement(h, a === !0, l.callback);
      },
      /**
      ...
      highlightElement: function(i, a, u) {
        var l = o.util.getLanguage(i), d = o.languages[l];
        o.util.setLanguage(i, l);
        var h = i.parentElement;
        h && h.nodeName.toLowerCase() === "pre" && o.util.setLanguage(h, l);
        var b = i.textContent, f = {
          element: i,
          language: l,
          grammar: d,
          code: b
        ...
        function F(B) {
          f.highlightedCode = B, o.hooks.run("before-insert", f), f.element.innerHTML = f.highlightedCode, o.hooks.run("after-highlight", f), o.hooks.run("complete", f), u && u.call(f.element);
        }
        if (o.hooks.run("before-sanity-check", f), h = f.element.parentElement, h && h.nodeName.toLowerCase() === "pre" && !h.hasAttribute("tabindex") && h.setAttribute("tabindex", "0"), !f.code) {
      ...
      tokenize: function(i, a) {
        ...
        var d = new E();
        return x(d, d.head, i), T(i, d, a, d.head, 0), C(d);
      },
      /**
       * @namespace
       * @memberof Prism
    ...
    m.stringify = function i(a, u) {
      ...
      h && (Array.isArray(h) ? Array.prototype.push.apply(d.classes, h) : d.classes.push(h)), o.hooks.run("wrap", d);
      var b = "";
      for (var f in d.attributes)
        b += " " + f + '="' + (d.attributes[f] || "").replace(/"/g, "&quot;") + '"';
      return "<" + d.tag + ' class="' + d.classes.join(" ") + '"' + b + ">" + d.content + "</" + d.tag + ">";
    };
    function v(i, a, u, l) {
      i.lastIndex = a;
      var d = i.exec(u);
      if (d && l && d[1]) {
        var h = d[1].length;
        d.index += h, d[0] = d[0].slice(h);
      }
      return d;
    }
    function T(i, a, u, l, d, h) {
      for (var b in u)
        if (!(!u.hasOwnProperty(b) || !u[b])) {
          ...
          for (var F = 0; F < f.length; ++F) {
            if (h && h.cause == b + "," + F)
              return;
            var A = f[F], B = A.inside, D = !!A.lookbehind, G = !!A.greedy, re = A.alias;
            if (G && !A.pattern.global) {
              var ae = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, ae + "g");
            }
            for (var q = A.pattern || A, S = l.next, M = d; S !== a.tail && !(h && M >= h.reach); M += S.value.length, S = S.next) {
              var P = S.value;
              ...
              if (!(P instanceof m)) {
                var I = 1, L;
                if (G) {
                  if (L = v(q, M, i, D), !L || L.index >= i.length)
                    break;
                  var $ = L.index, ie = L.index + L[0].length, k = M;
                  for (k += S.value.length; $ >= k; )
                    S = S.next, k += S.value.length;
                  if (k -= S.value.length, M = k, S.value instanceof m)
                    continue;
                  for (var U = S; U !== a.tail && (k < ie || typeof U.value == "string"); U = U.next)
                    I++, k += U.value.length;
                  I--, P = i.slice(M, k), L.index -= M;
                } else if (L = v(q, 0, P, D), !L)
                  continue;
                var $ = L.index, z = L[0], H = P.slice(0, $), W = P.slice($ + z.length), X = M + P.length;
                h && X > h.reach && (h.reach = X);
                var O = S.prev;
                H && (O = x(a, O, H), M += H.length), _(a, O, I);
                var se = new m(b, B ? o.tokenize(z, B) : z, re, z);
                if (S = x(a, O, se), W && x(a, S, W), I > 1) {
                  var V = {
                    cause: b + "," + F,
                    reach: X
                  };
                  T(i, a, u, S.prev, M, V), h && V.reach > h.reach && (h.reach = V.reach);
                }
              }
            }
          }
    ...
    if (!r.document)
      return r.addEventListener && (o.disableWorkerMessageHandler || r.addEventListener("message", function(i) {
        ...
        r.postMessage(o.highlight(l, o.languages[u], u)), d && r.close();
      }, !1)), o;
    ...
    if (!o.manual) {
      ...
      R === "loading" || R === "interactive" && y && y.defer ? document.addEventListener("DOMContentLoaded", w) : window.requestAnimationFrame ? window.requestAnimationFrame(w) : window.setTimeout(w, 16);
    }
    return o;
  }(e);
  n.exports && (n.exports = t), typeof Z < "u" && (Z.Prism = t), t.languages.markup = {
    ...
    var r = "Loading…", s = function(y, w) {
      ...
      h: "c",
      tex: "latex"
    }, o = "data-src-status", m = "loading", v = "loaded", T = "failed", E = "pre[data-src]:not([" + o + '="' + v + '"]):not([' + o + '="' + m + '"])';
    function x(y, w, R) {
      var i = new XMLHttpRequest();
      i.open("GET", y, !0), i.onreadystatechange = function() {
        i.readyState == 4 && (i.status < 400 && i.responseText ? w(i.responseText) : i.status >= 400 ? R(s(i.status, i.statusText)) : R(c));
      }, i.send(null);
    }
    function _(y) {
      var w = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(y || "");
      if (w) {
        var R = Number(w[1]), i = w[2], a = w[3];
        return i ? a ? [R, Number(a)] : [R, void 0] : [R, R];
      }
    }
    t.hooks.add("before-highlightall", function(y) {
      ...
      if (w.matches(E)) {
        ...
        R.textContent = r;
        var i = w.getAttribute("data-src"), a = y.language;
        if (a === "none") {
        ...
        l && l.loadLanguages(a), x(
          ...
          function(d) {
            w.setAttribute(o, v);
            var h = _(w.getAttribute("data-range"));
            if (h) {
              var b = d.split(/\r\n?|\n/g), f = h[0], F = h[1] == null ? b.length : h[1];
              f < 0 && (f += b.length), f = Math.max(0, Math.min(f - 1, b.length)), F < 0 && (F += b.length), F = Math.max(0, Math.min(F, b.length)), d = b.slice(f, F).join(`
`), w.hasAttribute("data-start") || w.setAttribute("data-start", String(f + 1));
            }
            R.textContent = d, t.highlightElement(R);
          },
          function(d) {
            w.setAttribute(o, T), R.textContent = d;
          }
        );
      }
      ...
      highlight: function(w) {
        for (var R = (w || document).querySelectorAll(E), i = 0, a; a = R[i++]; )
...
class xe {
  ...
  buildCodePanel() {
    ...
    `, T = (E) => {
      const x = e[E];
      o = x.source;
      const _ = document.createElement("pre"), C = document.createElement("code");
      C.className = "language-cpp", C.textContent = x.source, _.appendChild(C), c.innerHTML = "", c.appendChild(_), te.highlightElement(C);
    };
    p.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(o), p.innerHTML = v, p.classList.add("copied"), setTimeout(() => {
      ...
      _.className = "tab-button", _.textContent = E.name, x === 0 && _.classList.add("active"), _.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((C) => C.classList.remove("active")), _.classList.add("active"), T(x);
      }), s.appendChild(_);
    }), this.codePanel.appendChild(s), this.codePanel.appendChild(p), this.codePanel.appendChild(c), e.length > 0 && T(0);
  }
}
class ye {
  constructor(e) {
  ...
  buildTabBar() {
    ...
    for (const v of r) {
      const T = this.project.passes[v];
      T && t.push({
        name: `${v.toLowerCase()}.glsl`,
        isShader: !1,
        source: T.glslSource
      });
    }
...
`), x = document.createElement("pre"), _ = document.createElement("div");
        _.className = "tabbed-line-numbers", _.innerHTML = E.map((w, R) => R + 1).join(`
`);
        const C = document.createElement("div");
        C.className = "tabbed-code-content";
        const y = document.createElement("code");
        y.className = "language-cpp", y.textContent = c, C.appendChild(y), x.appendChild(_), x.appendChild(C), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(x), te.highlightElement(y);
      }
    };
    return t.forEach((v, T) => {
      const E = document.createElement("button");
      E.className = "tabbed-tab-button", v.isShader && E.classList.add("shader-tab"), E.textContent = v.name, T === 0 && E.classList.add("active"), E.addEventListener("click", () => m(T)), e.appendChild(E);
    }), e;
  }
}
function _e(n, e) {
...
async function we(n, e, t, r) {
  const s = `/demos/${n}/shadertoy.config.json`, c = `/demos/${n}/config.json`, p = s in t ? s : c in t ? c : null;
  if (p) {
    const o = await t[p]();
    return o.passes ? Fe(n, o, e, r) : Q(n, e, o);
  } else
    return Q(n, e);
}
async function Q(n, e, t) {
  ...
  const s = await e[r](), c = (t == null ? void 0 : t.layout) || "tabbed", p = (t == null ? void 0 : t.controls) ?? !0, o = ((m = t == null ? void 0 : t.meta) == null ? void 0 : m.title) || n.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" ");
  return {
    root: `/demos/${n}`,
    meta: {
      title: o,
      author: ((v = t == null ? void 0 : t.meta) == null ? void 0 : v.author) || null,
...
async function Fe(n, e, t, r) {
  ...
  for (const h of p) {
    ...
    if (b)
      for (const f of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const F = (y = b.channels) == null ? void 0 : y[f];
        F && "texture" in F && c.add(F.texture);
      }
  }
  const o = [], m = /* @__PURE__ */ new Map();
  for (const h of c) {
    const b = `/demos/${n}/${h.replace(/^\.\//, "")}`;
    if (!(b in r))
      throw new Error(`Texture not found: ${h} (expected at ${b})`);
    const f = await r[b](), F = h.split("/").pop().replace(/\.[^.]+$/, "");
    o.push({
      name: F,
      source: f,
      filter: "linear",
  ...
  for (const h of p) {
    ...
    }, F = b.source || f[h], A = `/demos/${n}/${F}`;
    if (!(A in t))
      throw new Error(`Missing shader file: ${A}`);
    const B = await t[A](), D = [
      j((w = b.channels) == null ? void 0 : w.iChannel0, m),
      j((R = b.channels) == null ? void 0 : R.iChannel1, m),
      j((i = b.channels) == null ? void 0 : i.iChannel2, m),
      j((a = b.channels) == null ? void 0 : a.iChannel3, m)
    ];
    v[h] = {
      name: h,
      glslSource: B,
  ...
  const T = ((u = e.meta) == null ? void 0 : u.title) || n.split("-").map((h) => h.charAt(0).toUpperCase() + h.slice(1)).join(" "), E = ((l = e.meta) == null ? void 0 : l.author) || null, x = ((d = e.meta) == null ? void 0 : d.description) || null, _ = e.layout || "tabbed", C = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Ce = "course/day3/hook-animated-tiling";
async function Re() {
  return we(Ce, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/hook-animated-tiling/image.glsl": () => Promise.resolve().then(() => Se).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day3/hook-animated-tiling/config.json": () => Promise.resolve().then(() => Be).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function ke(n) {
  ...
  return s.hasErrors() || s.start(), {
    ...
    destroy: () => {
      var c;
      (c = s.stop) == null || c.call(s);
    }
  };
}
const Ae = `// Color palette
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  __proto__: null,
  default: Ae
}, Symbol.toStringTag, { value: "Module" })), ne = "centered", Le = {
  layout: ne
