...
class ge {
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      r.deleteTexture(s.currentTexture), r.deleteTexture(s.previousTexture), r.deleteFramebuffer(s.framebuffer), s.currentTexture = O(r, e, t), s.previousTexture = O(r, e, t), s.framebuffer = Y(r, s.currentTexture);
  }
  /**
  ...
  initRuntimePasses() {
    ...
    for (const c of s) {
      ...
      try {
        ...
        }, T = O(e, this._width, this._height), E = O(e, this._width, this._height), x = Y(e, T), _ = {
          name: c,
...
`), s = 3, c = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(c, p).map((b, T) => {
      const E = c + T + 1, x = E === e, _ = String(E).padStart(4, " "), C = this.escapeHTML(b);
      ...
      for (var v in u)
        if (!(!u.hasOwnProperty(v) || !u[v])) {
          ...
          for (var F = 0; F < f.length; ++F) {
            if (d && d.cause == v + "," + F)
              return;
            var R = f[F], M = R.inside, D = !!R.lookbehind, V = !!R.greedy, ne = R.alias;
            ...
            for (var q = R.pattern || R, S = l.next, B = h; S !== a.tail && !(d && B >= d.reach); B += S.value.length, S = S.next) {
              ...
              if (!(P instanceof m)) {
                ...
                var N = S.prev;
                H && (N = x(a, N, H), B += H.length), _(a, N, I);
                var ie = new m(v, M ? o.tokenize(z, M) : z, ne, z);
                if (S = x(a, N, ie), W && x(a, S, W), I > 1) {
                  var G = {
                    cause: v + "," + F,
                    reach: X
                  };
...
async function we(n, e, t, r) {
  ...
  for (const d of p) {
    ...
    const M = await t[R](), D = [
      ...
      j((i = v.channels) == null ? void 0 : i.iChannel2, m),
  ...
  const T = ((u = e.meta) == null ? void 0 : u.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), E = ((l = e.meta) == null ? void 0 : l.author) || null, x = ((h = e.meta) == null ? void 0 : h.description) || null, _ = e.layout || "tabbed", C = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Fe = "course/day3/hyp-tiling-237";
async function Ce() {
  return _e(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/hyp-tiling-237/image.glsl": () => Promise.resolve().then(() => Re).then((r) => r.default)
...
async function Le(n) {
  ...
  const t = await Ce(), r = ye(t.layout, {
    container: e,
    project: t
  }), s = new Ee({
