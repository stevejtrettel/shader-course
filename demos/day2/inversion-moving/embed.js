...
class ge {
  ...
  step(e, t) {
    ...
    const c = [this._width, this._height, 1], p = this._time, o = s, m = this._frame, b = t;
    r.viewport(0, 0, this._width, this._height);
    const T = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const E of T) {
      const _ = this._passes.find((y) => y.name === E);
      _ && (this.executePass(_, {
        iResolution: c,
        iTime: p,
        iTimeDelta: o,
        iFrame: m,
        iMouse: b
      }), this.swapPassTextures(_));
    }
    this._frame += 1;
  }
  /**
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      r.deleteTexture(s.currentTexture), r.deleteTexture(s.previousTexture), r.deleteFramebuffer(s.framebuffer), s.currentTexture = j(r, e, t), s.previousTexture = j(r, e, t), s.framebuffer = Y(r, s.currentTexture);
  }
  /**
  ...
  initRuntimePasses() {
    ...
    for (const c of s) {
      ...
      try {
        ...
        }, T = j(e, this._width, this._height), E = j(e, this._width, this._height), _ = Y(e, T), y = {
          name: c,
        ...
        if (E && this.project.commonSource) {
          const A = parseInt(E[1], 10), x = T.boilerplateLinesBeforeCommon + 2, w = x + T.commonLineCount - 1;
          A >= x && A <= w && (_ = !0, y = A - x + 1);
        }
...
class Ee {
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((b) => b.isFromCommon), r = e.filter((b) => !b.isFromCommon), o = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: b, error: T, source: E, isFromCommon: _, originalLine: y }) => {
      const A = T.replace(`Shader compilation failed:
`, "");
      let x = A;
      return _ && y !== null && (x = A.replace(/Line \d+:/, `Line ${y}:`), x = x.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${y}:`)), {
        passName: _ ? "common.glsl" : b,
        error: this.parseShaderError(x),
        codeContext: _ ? this.extractCodeContextFromCommon(y) : this.extractCodeContext(x, E)
      };
...
`), p = 3, o = Math.max(0, s - p - 1), m = Math.min(c.length, s + p);
    return c.slice(o, m).map((E, _) => {
      const y = o + _ + 1, A = y === s, x = String(y).padStart(4, " "), w = this.escapeHTML(E);
      return A ? `<span class="error-line-highlight">${x} │ ${w}</span>` : `<span class="context-line">${x} │ ${w}</span>`;
    }).join("");
  }
  /**
...
`), s = 3, c = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(c, p).map((b, T) => {
      const E = c + T + 1, _ = E === e, y = String(E).padStart(4, " "), A = this.escapeHTML(b);
      return _ ? `<span class="error-line-highlight">${y} │ ${A}</span>` : `<span class="context-line">${y} │ ${A}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
  ...
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, c = 0, p = {}, o = {
        ...
        clone: function i(a, u) {
          ...
          switch (o.util.type(a)) {
            case "Object":
              ...
              {}, u[h] = l;
              for (var d in a)
                a.hasOwnProperty(d) && (l[d] = i(a[d], u));
              return (
                /** @type {any} */
                l
              );
      ...
      languages: {
        ...
        insertBefore: function(i, a, u, l) {
          ...
          var F = l[i];
          return l[i] = d, o.languages.DFS(o.languages, function(C, M) {
            M === F && C != i && (this[C] = d);
          }), d;
        },
        // Traverse a language definition with Depth First Search
        DFS: function i(a, u, l, h) {
          h = h || {};
          var d = o.util.objId;
          for (var v in a)
            if (a.hasOwnProperty(v)) {
              ...
              F === "Object" && !h[d(f)] ? (h[d(f)] = !0, i(f, u, null, h)) : F === "Array" && !h[d(f)] && (h[d(f)] = !0, i(f, u, v, h));
            }
        }
      },
      plugins: {},
      ...
      tokenize: function(i, a) {
        ...
        var h = new E();
        return _(h, h.head, i), T(i, h, a, h.head, 0), A(h);
      },
      /**
       * @namespace
       * @memberof Prism
          ...
          for (var F = 0; F < f.length; ++F) {
            ...
            for (var q = C.pattern || C, S = l.next, B = h; S !== a.tail && !(d && B >= d.reach); B += S.value.length, S = S.next) {
              ...
              if (!(P instanceof m)) {
                var $ = 1, L;
                if (V) {
                  if (L = b(q, B, i, D), !L || L.index >= i.length)
                    break;
                  var I = L.index, ae = L.index + L[0].length, k = B;
                  for (k += S.value.length; I >= k; )
                  ...
                  for (var U = S; U !== a.tail && (k < ae || typeof U.value == "string"); U = U.next)
                    $++, k += U.value.length;
                  $--, P = i.slice(B, k), L.index -= B;
                } else if (L = b(q, 0, P, D), !L)
                  continue;
                var I = L.index, N = L[0], H = P.slice(0, I), W = P.slice(I + N.length), X = B + P.length;
                d && X > d.reach && (d.reach = X);
                var O = S.prev;
                H && (O = _(a, O, H), B += H.length), y(a, O, $);
                var ie = new m(v, M ? o.tokenize(N, M) : N, ne, N);
                if (S = _(a, O, ie), W && _(a, S, W), $ > 1) {
                  var G = {
                    cause: v + "," + F,
                    reach: X
                  };
                  T(i, a, u, S.prev, B, G), d && G.reach > d.reach && (d.reach = G.reach);
                }
              }
            }
          }
    ...
    function _(i, a, u) {
      ...
      return a.next = h, l.prev = h, i.length++, h;
    }
    function y(i, a, u) {
      for (var l = a.next, h = 0; h < u && l !== i.tail; h++)
        l = l.next;
      a.next = l, l.prev = a, i.length -= h;
    }
    function A(i) {
    ...
    if (!r.document)
      return r.addEventListener && (o.disableWorkerMessageHandler || r.addEventListener("message", function(i) {
        var a = JSON.parse(i.data), u = a.language, l = a.code, h = a.immediateClose;
    ...
    if (!o.manual) {
      ...
      R === "loading" || R === "interactive" && x && x.defer ? document.addEventListener("DOMContentLoaded", w) : window.requestAnimationFrame ? window.requestAnimationFrame(w) : window.setTimeout(w, 16);
    }
    return o;
  }(e);
  n.exports && (n.exports = t), typeof Z < "u" && (Z.Prism = t), t.languages.markup = {
    ...
    var r = "Loading…", s = function(x, w) {
      return "✖ Error " + x + " while fetching file: " + w;
    ...
    t.hooks.add("before-highlightall", function(x) {
      ...
      if (w.matches(E)) {
        x.code = "", w.setAttribute(o, m);
        var R = w.appendChild(document.createElement("CODE"));
        R.textContent = r;
        var i = w.getAttribute("data-src"), a = x.language;
        if (a === "none") {
...
class Te {
  ...
  buildCodePanel() {
    ...
    `, T = (E) => {
      const _ = e[E];
      o = _.source;
      const y = document.createElement("pre"), A = document.createElement("code");
      A.className = "language-cpp", A.textContent = _.source, y.appendChild(A), c.innerHTML = "", c.appendChild(y), te.highlightElement(A);
    };
    p.addEventListener("click", async () => {
      ...
      y.className = "tab-button", y.textContent = E.name, _ === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((A) => A.classList.remove("active")), y.classList.add("active"), T(_);
      }), s.appendChild(y);
...
class _e {
  ...
  buildTabBar() {
    ...
    const m = (b) => {
      const T = t[b];
      if (e.querySelectorAll(".tabbed-tab-button").forEach((E, _) => {
        E.classList.toggle("active", _ === b);
      }), T.isShader)
        ...
        const E = c.split(`
`), _ = document.createElement("pre"), y = document.createElement("div");
        y.className = "tabbed-line-numbers", y.innerHTML = E.map((w, R) => R + 1).join(`
`);
        const A = document.createElement("div");
        A.className = "tabbed-code-content";
        const x = document.createElement("code");
        x.className = "language-cpp", x.textContent = c, A.appendChild(x), _.appendChild(y), _.appendChild(A), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(_), te.highlightElement(x);
      }
    };
...
async function we(n, e, t, r) {
  ...
  for (const d of p) {
    ...
    if (v)
      for (const f of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const F = (x = v.channels) == null ? void 0 : x[f];
  ...
  for (const d of p) {
    ...
    const M = await t[C](), D = [
      ...
      z((R = v.channels) == null ? void 0 : R.iChannel1, m),
      z((i = v.channels) == null ? void 0 : i.iChannel2, m),
  ...
  const T = ((u = e.meta) == null ? void 0 : u.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), E = ((l = e.meta) == null ? void 0 : l.author) || null, _ = ((h = e.meta) == null ? void 0 : h.description) || null, y = e.layout || "tabbed", A = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
    meta: { title: T, author: E, description: _ },
    layout: y,
    controls: A,
    commonSource: s,
    passes: b,
...
const Fe = "course/day2/inversion-moving";
async function Ae() {
  return ye(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day2/inversion-moving/image.glsl": () => Promise.resolve().then(() => Ce).then((r) => r.default)
...
async function Le(n) {
  ...
  const t = await Ae(), r = xe(t.layout, {
    container: e,
    project: t
  }), s = new Ee({
...
const Re = `vec2 normalize_coord(vec2 fragCoord) {
