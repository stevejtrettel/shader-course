    ...
    for (const c of i) {
      ...
      try {
        ...
        if (E && this.project.commonSource) {
          const A = parseInt(E[1], 10), _ = T.boilerplateLinesBeforeCommon + 2, w = _ + T.commonLineCount - 1;
          A >= _ && A <= w && (x = !0, y = A - _ + 1);
        }
...
`), p = 3, o = Math.max(0, i - p - 1), m = Math.min(c.length, i + p);
    return c.slice(o, m).map((E, x) => {
      const y = o + x + 1, A = y === i, _ = String(y).padStart(4, " "), w = this.escapeHTML(E);
      return A ? `<span class="error-line-highlight">${_} │ ${w}</span>` : `<span class="context-line">${_} │ ${w}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
  ...
  var t = function(r) {
    ...
    if (!o.manual) {
      ...
      C === "loading" || C === "interactive" && _ && _.defer ? document.addEventListener("DOMContentLoaded", w) : window.requestAnimationFrame ? window.requestAnimationFrame(w) : window.setTimeout(w, 16);
    }
    return o;
  }(e);
  n.exports && (n.exports = t), typeof Z < "u" && (Z.Prism = t), t.languages.markup = {
    ...
    function x(_, w, C) {
      ...
      s.open("GET", _, !0), s.onreadystatechange = function() {
        s.readyState == 4 && (s.status < 400 && s.responseText ? w(s.responseText) : s.status >= 400 ? C(i(s.status, s.statusText)) : C(c));
      }, s.send(null);
    }
    function y(_) {
      var w = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(_ || "");
      if (w) {
        var C = Number(w[1]), s = w[2], a = w[3];
    ...
    t.hooks.add("before-highlightall", function(_) {
      ...
      if (w.matches(E)) {
        ...
        t.util.setLanguage(C, a), t.util.setLanguage(w, a);
...
`), w.hasAttribute("data-start") || w.setAttribute("data-start", String(f + 1));
          ...
          function(h) {
            w.setAttribute(o, T), C.textContent = h;
          }
        );
      }
      ...
      highlight: function(w) {
        for (var C = (w || document).querySelectorAll(E), s = 0, a; a = C[s++]; )
...
`), x = document.createElement("pre"), y = document.createElement("div");
        y.className = "tabbed-line-numbers", y.innerHTML = E.map((w, C) => C + 1).join(`
`);
...
const Ce = "course/day4/wave-interactive";
async function Re() {
  return Fe(Ce, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/wave-interactive/bufferA.glsl": () => Promise.resolve().then(() => Le).then((r) => r.default),
    "/demos/course/day4/wave-interactive/image.glsl": () => Promise.resolve().then(() => Be).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/wave-interactive/config.json": () => Promise.resolve().then(() => Pe).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function De(n) {
