...
function Z(n, e, t) {
  const r = Y(n, n.VERTEX_SHADER, e), i = Y(n, n.FRAGMENT_SHADER, t), l = n.createProgram();
  if (!l)
    throw new Error("Failed to create program object");
  if (n.attachShader(l, r), n.attachShader(l, i), n.linkProgram(l), !n.getProgramParameter(l, n.LINK_STATUS)) {
    const a = n.getProgramInfoLog(l);
    throw n.deleteProgram(l), n.deleteShader(r), n.deleteShader(i), new Error(`Program linking failed:
${a}`);
  }
  return n.detachShader(l, r), n.detachShader(l, i), n.deleteShader(r), n.deleteShader(i), l;
}
function fe(n) {
...
function Ee(n, e, t, r) {
  ...
  for (let a = 0; a < 256; a++) {
    ...
    s[v + 0] = m ? 1 : 0, s[v + 1] = m ? 1 : 0, s[v + 2] = m ? 1 : 0, s[v + 3] = 1;
    const w = (2 * 256 + a) * 4;
    s[w + 0] = g, s[w + 1] = g, s[w + 2] = g, s[w + 3] = 1;
  }
...
class Te {
  ...
  step(e, t) {
    ...
    const l = [this._width, this._height, 1], s = this._time, a = i, m = this._frame, g = t;
  ...
  resize(e, t) {
    ...
    for (const i of this._passes)
      r.deleteTexture(i.currentTexture), r.deleteTexture(i.previousTexture), r.deleteFramebuffer(i.framebuffer), i.currentTexture = O(r, e, t), i.previousTexture = O(r, e, t), i.framebuffer = J(r, i.currentTexture);
  }
  /**
  ...
  recompilePass(e, t) {
    ...
    try {
      const a = Z(r, Q, s);
      r.deleteProgram(i.uniforms.program);
      const m = {
        program: a,
      ...
      return i.uniforms = m, l.glslSource = t, this._compilationErrors = this._compilationErrors.filter((g) => g.passName !== e), { success: !0 };
    } catch (a) {
  ...
  recompileCommon(e) {
    ...
    if (r.length > 0) {
      ...
      for (const l of i) {
        const s = this.project.passes[l];
        s && (r.some((a) => a.passName === l) || this.recompilePass(l, s.glslSource));
      }
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, r = fe(e), i = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const l of i) {
      const s = t.passes[l];
      if (!s) continue;
      ...
      try {
        ...
        if (w && this.project.commonSource) {
          const A = parseInt(w[1], 10), y = v.boilerplateLinesBeforeCommon + 2, E = y + v.commonLineCount - 1;
          A >= y && A <= E && (x = !0, C = A - y + 1);
        }
...
`);
    ...
    return r.push("// User shader code"), r.push(i), r.push(""), r.push(`// Main wrapper
out vec4 fragColor;
...
`);
  ...
  preprocessCubemapTextures(e, t) {
    ...
    if (t.forEach((l, s) => {
      l.kind === "texture" && l.cubemap && r.add(`iChannel${s}`);
    }), r.size === 0)
      return e;
    const i = /texture\s*\(\s*(iChannel[0-3])\s*,\s*([^)]+)\)/g;
    return e.replace(i, (l, s, a) => r.has(s) ? `texture(${s}, _st_dirToEquirect(${a}))` : l);
  }
  // ===========================================================================
  // Pass Execution
  // ===========================================================================
  ...
  bindChannelTextures(e) {
    const t = this.gl;
    for (let r = 0; r < 4; r++) {
      const i = e.projectChannels[r], l = this.resolveChannelTexture(i);
      t.activeTexture(t.TEXTURE0 + r), t.bindTexture(t.TEXTURE_2D, l);
      const s = e.uniforms.iChannel[r];
      s && t.uniform1i(s, r);
    }
  }
  /**
...
class xe {
  constructor(e) {
    ...
    this.gl = t, this.updateCanvasSize(), this.engine = new Te({
      gl: this.gl,
      project: e.project
  ...
  setupMouseTracking() {
    const e = (r) => {
      const i = this.canvas.getBoundingClientRect(), l = (r.clientX - i.left) * this.pixelRatio, s = (i.height - (r.clientY - i.top)) * this.pixelRatio;
...
`, "");
      let y = A;
      return x && C !== null && (y = A.replace(/Line \d+:/, `Line ${C}:`), y = y.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${C}:`)), {
...
`);
  ...
  extractCodeContext(e, t) {
    ...
    if (!r) return null;
    const i = parseInt(r[1], 10), l = t.split(`
`), s = 3, a = Math.max(0, i - s - 1), m = Math.min(l.length, i + s);
    return l.slice(a, m).map((w, x) => {
      const C = a + x + 1, A = C === i, y = String(C).padStart(4, " "), E = this.escapeHTML(w);
  ...
  extractCodeContextFromCommon(e) {
    ...
    if (!t) return null;
    const r = t.split(`
`), i = 3, l = Math.max(0, e - i - 1), s = Math.min(r.length, e + i);
    return r.slice(l, s).map((g, v) => {
...
class Fe {
  ...
  buildTabBar() {
    ...
    const l = async (s) => {
      this.saveCurrentEditorContent();
      const a = this.tabs[s];
      if (this.activeTabIndex = s, t.querySelectorAll(".tabbed-tab-button").forEach((m, g) => {
        m.classList.toggle("active", g === s);
    ...
    return this.tabs.forEach((s, a) => {
      ...
      m.className = "tabbed-tab-button", s.kind === "shader" ? m.classList.add("shader-tab") : s.kind === "image" && m.classList.add("image-tab"), m.textContent = s.name, a === 0 && m.classList.add("active"), m.addEventListener("click", () => l(a)), t.appendChild(m);
...
async function Re(n, e, t, r) {
  const i = `/demos/${n}/config.json`;
  if (i in t) {
    const s = await t[i]();
    return s.Image || s.BufferA || s.BufferB || s.BufferC || s.BufferD ? Be(n, s, e, r) : ee(n, e, s);
  } else
    return ee(n, e);
}
async function ee(n, e, t) {
  const r = `/demos/${n}/image.glsl`, i = I(e, r);
  if (!i)
    throw new Error(`Demo '${n}' not found. Expected ${r}`);
  const l = await e[i](), s = (t == null ? void 0 : t.layout) || "tabbed", a = (t == null ? void 0 : t.controls) ?? !0, m = (t == null ? void 0 : t.title) || n.split("-").map((g) => g.charAt(0).toUpperCase() + g.slice(1)).join(" ");
  return {
    root: `/demos/${n}`,
    meta: {
      title: m,
...
async function Be(n, e, t, r) {
  ...
  let l = null;
  if (e.common) {
    const E = `/demos/${n}/${e.common}`, T = I(t, E);
    T && (l = await t[T]());
  } else {
    const E = `/demos/${n}/common.glsl`, T = I(t, E);
    T && (l = await t[T]());
  }
  ...
  for (const E of s) {
    const T = `/demos/${n}/${E.replace(/^\.\//, "")}`, c = I(r, T);
    if (!c)
  ...
  for (const E of a) {
    ...
    }, o = T.source || c[E], u = `/demos/${n}/${o}`, d = I(t, u);
    if (!d)
  ...
  const w = e.title || n.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" "), x = e.author || null, C = e.description || null, A = e.layout || "tabbed", y = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Le = "course/day4/texture-wavy";
async function ke() {
  return Re(Le, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/texture-wavy/image.glsl": () => Promise.resolve().then(() => Ne).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/texture-wavy/config.json": () => Promise.resolve().then(() => je).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function He(n) {
...
class Me {
  ...
  buildTabs() {
    ...
    for (const r of this.project.textures) {
      const i = r.source.split("/").pop() || r.source;
      this.tabs.push({
        kind: "image",
        name: i,
        url: r.source
  ...
  async showTab(e) {
    ...
    if (this.tabBar.querySelectorAll(".editor-tab-button").forEach((r, i) => {
      ...
      i.src = t.url, i.alt = t.name, r.appendChild(i), this.contentArea.appendChild(r);
    }
  }
  saveCurrentEditorContent() {
    if (this.editorInstance) {
...
function Ie(n, e, t) {
  ...
  m.className = "language-cpp", a.appendChild(m), l.appendChild(s), l.appendChild(a), r.appendChild(i), r.appendChild(l), n.appendChild(r);
  function g() {
    const x = s.value;
    m.textContent = x + `
`, De.highlightElement(m);
    const C = x.split(`
`);
    i.innerHTML = C.map((A, y) => `<span>${y + 1}</span>`).join(""), t && t(x);
  }
  function v() {
  ...
  function w(x) {
    if (x.key === "Tab") {
      x.preventDefault();
      const C = s.selectionStart, A = s.selectionEnd, y = s.value;
      s.value = y.substring(0, C) + "  " + y.substring(A), s.selectionStart = s.selectionEnd = C + 2, g();
    }
  }
  return s.addEventListener("input", g), s.addEventListener("scroll", v), s.addEventListener("keydown", w), g(), {
    getSource: () => s.value,
    setSource: (x) => {
      s.value = x, g();
    },
    destroy: () => {
      s.removeEventListener("input", g), s.removeEventListener("scroll", v), s.removeEventListener("keydown", w), n.removeChild(r);
    }
  };
}
