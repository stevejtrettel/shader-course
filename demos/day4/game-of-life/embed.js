    ...
    for (const c of i) {
      ...
      try {
        ...
        if (E && this.project.commonSource) {
          const A = parseInt(E[1], 10), _ = T.boilerplateLinesBeforeCommon + 2, F = _ + T.commonLineCount - 1;
          A >= _ && A <= F && (x = !0, y = A - _ + 1);
        }
...
`, "");
      let _ = A;
      return x && y !== null && (_ = A.replace(/Line \d+:/, `Line ${y}:`), _ = _.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${y}:`)), {
...
`), p = 3, o = Math.max(0, i - p - 1), m = Math.min(c.length, i + p);
    return c.slice(o, m).map((E, x) => {
      const y = o + x + 1, A = y === i, _ = String(y).padStart(4, " "), F = this.escapeHTML(E);
      return A ? `<span class="error-line-highlight">${_} │ ${F}</span>` : `<span class="context-line">${_} │ ${F}</span>`;
    }).join("");
  }
  /**
...
`), i = 3, c = Math.max(0, e - i - 1), p = Math.min(r.length, e + i);
    return r.slice(c, p).map((b, T) => {
      const E = c + T + 1, x = E === e, y = String(E).padStart(4, " "), A = this.escapeHTML(b);
      return x ? `<span class="error-line-highlight">${y} │ ${A}</span>` : `<span class="context-line">${y} │ ${A}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
  ...
  var t = function(r) {
    var i = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, c = 0, p = {}, o = {
        ...
        DFS: function s(a, u, l, h) {
          ...
          for (var v in a)
            if (a.hasOwnProperty(v)) {
              u.call(a, v, a[v], l || v);
              var f = a[v], w = o.util.type(f);
              w === "Object" && !h[d(f)] ? (h[d(f)] = !0, s(f, u, null, h)) : w === "Array" && !h[d(f)] && (h[d(f)] = !0, s(f, u, v, h));
            }
        }
      },
      plugins: {},
      ...
      highlightElement: function(s, a, u) {
        ...
        if (a && r.Worker) {
          ...
          w(o.highlight(f.code, f.grammar, f.language));
      },
      /**
      ...
      tokenize: function(s, a) {
        ...
        var h = new E();
        return x(h, h.head, s), T(s, h, a, h.head, 0), A(h);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var v in u)
        if (!(!u.hasOwnProperty(v) || !u[v])) {
          ...
          for (var w = 0; w < f.length; ++w) {
            if (d && d.cause == v + "," + w)
              return;
            var R = f[w], M = R.inside, D = !!R.lookbehind, V = !!R.greedy, ae = R.alias;
    ...
    if (!o.manual) {
      ...
      C === "loading" || C === "interactive" && _ && _.defer ? document.addEventListener("DOMContentLoaded", F) : window.requestAnimationFrame ? window.requestAnimationFrame(F) : window.setTimeout(F, 16);
    }
    return o;
  }(e);
  n.exports && (n.exports = t), typeof Z < "u" && (Z.Prism = t), t.languages.markup = {
    ...
    var r = "Loading…", i = function(_, F) {
      return "✖ Error " + _ + " while fetching file: " + F;
    ...
    function x(_, F, C) {
      ...
      s.open("GET", _, !0), s.onreadystatechange = function() {
        s.readyState == 4 && (s.status < 400 && s.responseText ? F(s.responseText) : s.status >= 400 ? C(i(s.status, s.statusText)) : C(c));
      }, s.send(null);
    }
    function y(_) {
      var F = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(_ || "");
      if (F) {
        var C = Number(F[1]), s = F[2], a = F[3];
        return s ? a ? [C, Number(a)] : [C, void 0] : [C, C];
      }
    }
    t.hooks.add("before-highlightall", function(_) {
      ...
      if (F.matches(E)) {
        _.code = "", F.setAttribute(o, m);
        var C = F.appendChild(document.createElement("CODE"));
        C.textContent = r;
        var s = F.getAttribute("data-src"), a = _.language;
        if (a === "none") {
          var u = (/\.(\w+)$/.exec(s) || [, "none"])[1];
          a = p[u] || u;
        }
        t.util.setLanguage(C, a), t.util.setLanguage(F, a);
        var l = t.plugins.autoloader;
        l && l.loadLanguages(a), x(
          s,
          function(h) {
            F.setAttribute(o, b);
            var d = y(F.getAttribute("data-range"));
            if (d) {
              var v = h.split(/\r\n?|\n/g), f = d[0], w = d[1] == null ? v.length : d[1];
              f < 0 && (f += v.length), f = Math.max(0, Math.min(f - 1, v.length)), w < 0 && (w += v.length), w = Math.max(0, Math.min(w, v.length)), h = v.slice(f, w).join(`
`), F.hasAttribute("data-start") || F.setAttribute("data-start", String(f + 1));
            }
            C.textContent = h, t.highlightElement(C);
          },
          function(h) {
            F.setAttribute(o, T), C.textContent = h;
          }
        );
      }
      ...
      highlight: function(F) {
        for (var C = (F || document).querySelectorAll(E), s = 0, a; a = C[s++]; )
...
class _e {
  ...
  buildCodePanel() {
    ...
    `, T = (E) => {
      ...
      A.className = "language-cpp", A.textContent = x.source, y.appendChild(A), c.innerHTML = "", c.appendChild(y), te.highlightElement(A);
    };
    p.addEventListener("click", async () => {
      ...
      y.className = "tab-button", y.textContent = E.name, x === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        i.querySelectorAll(".tab-button").forEach((A) => A.classList.remove("active")), y.classList.add("active"), T(x);
      }), i.appendChild(y);
...
`), x = document.createElement("pre"), y = document.createElement("div");
        y.className = "tabbed-line-numbers", y.innerHTML = E.map((F, C) => C + 1).join(`
`);
        const A = document.createElement("div");
        A.className = "tabbed-code-content";
        const _ = document.createElement("code");
        _.className = "language-cpp", _.textContent = c, A.appendChild(_), x.appendChild(y), x.appendChild(A), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(x), te.highlightElement(_);
      }
    };
...
async function we(n, e, t, r) {
  ...
  if (p) {
    const o = await t[p]();
    return o.passes ? Ae(n, o, e, r) : Q(n, e, o);
  } else
    return Q(n, e);
}
...
async function Ae(n, e, t, r) {
  ...
  for (const d of p) {
    ...
    if (v)
      for (const f of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const w = (_ = v.channels) == null ? void 0 : _[f];
        w && "texture" in w && c.add(w.texture);
      }
  }
  ...
  for (const d of c) {
    ...
    const f = await r[v](), w = d.split("/").pop().replace(/\.[^.]+$/, "");
    o.push({
      name: w,
      source: f,
      filter: "linear",
  ...
  for (const d of p) {
    ...
    }, w = v.source || f[d], R = `/demos/${n}/${w}`;
    if (!(R in t))
      throw new Error(`Missing shader file: ${R}`);
    const M = await t[R](), D = [
      z((F = v.channels) == null ? void 0 : F.iChannel0, m),
      z((C = v.channels) == null ? void 0 : C.iChannel1, m),
  ...
  const T = ((u = e.meta) == null ? void 0 : u.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), E = ((l = e.meta) == null ? void 0 : l.author) || null, x = ((h = e.meta) == null ? void 0 : h.description) || null, y = e.layout || "tabbed", A = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Ce = "course/day4/game-of-life";
async function Re() {
  return we(Ce, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/game-of-life/bufferA.glsl": () => Promise.resolve().then(() => Le).then((r) => r.default),
    "/demos/course/day4/game-of-life/image.glsl": () => Promise.resolve().then(() => Be).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/game-of-life/config.json": () => Promise.resolve().then(() => Pe).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function De(n) {
  ...
  const t = await Re(), r = Fe(t.layout, {
    container: e,
    project: t
  }), i = new ve({
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  __proto__: null,
  default: Se
}, Symbol.toStringTag, { value: "Module" })), Me = `void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
