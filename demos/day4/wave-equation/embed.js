...
async function Le(r, e, t, n) {
  ...
  const w = e.title || r.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" "), x = e.author || null, C = e.description || null, A = e.layout || "tabbed", y = e.controls ?? !0;
  return {
    root: `/demos/${r}`,
...
const ke = "course/day4/wave-equation";
async function Me() {
  return Be(ke, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/wave-equation/bufferA.glsl": () => Promise.resolve().then(() => $e).then((n) => n.default),
    "/demos/course/day4/wave-equation/image.glsl": () => Promise.resolve().then(() => Oe).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/wave-equation/config.json": () => Promise.resolve().then(() => ze).then((n) => n.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Ge(r) {
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var F = 0; F < b.length; ++F) {
            if (m && m.cause == _ + "," + F)
              return;
            var S = b[F], k = S.inside, V = !!S.lookbehind, q = !!S.greedy, ue = S.alias;
            if (q && !S.pattern.global) {
              var de = S.pattern.toString().match(/[imsuy]*$/)[0];
              S.pattern = RegExp(S.pattern.source, de + "g");
            }
            for (var K = S.pattern || S, R = d.next, L = p; R !== o.tail && !(m && L >= m.reach); L += R.value.length, R = R.next) {
              var P = R.value;
...
`);
  ...
  return s.addEventListener("input", g), s.addEventListener("scroll", v), s.addEventListener("keydown", w), g(), {
    ...
    destroy: () => {
      s.removeEventListener("input", g), s.removeEventListener("scroll", v), s.removeEventListener("keydown", w), r.removeChild(n);
    }
  };
}
