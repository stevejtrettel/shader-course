...
`), s = 3, a = Math.max(0, i - s - 1), m = Math.min(l.length, i + s);
    return l.slice(a, m).map((w, x) => {
      const C = a + x + 1, A = C === i, T = String(C).padStart(4, " "), E = this.escapeHTML(w);
...
const Le = "course/day4/texture-flip";
async function ke() {
  return Re(Le, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/texture-flip/image.glsl": () => Promise.resolve().then(() => Ne).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/texture-flip/config.json": () => Promise.resolve().then(() => je).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function He(n) {
  ...
  n.exports && (n.exports = t), typeof te < "u" && (te.Prism = t), t.languages.markup = {
    ...
    function x(T, E, y) {
      ...
      c.open("GET", T, !0), c.onreadystatechange = function() {
        c.readyState == 4 && (c.status < 400 && c.responseText ? E(c.responseText) : c.status >= 400 ? y(i(c.status, c.statusText)) : y(l));
      }, c.send(null);
    }
    function C(T) {
...
`, De.highlightElement(m);
    const C = x.split(`
`);
    i.innerHTML = C.map((A, T) => `<span>${T + 1}</span>`).join(""), t && t(x);
  }
  function v() {
  ...
  function w(x) {
    if (x.key === "Tab") {
      x.preventDefault();
      const C = s.selectionStart, A = s.selectionEnd, T = s.value;
      s.value = T.substring(0, C) + "  " + T.substring(A), s.selectionStart = s.selectionEnd = C + 2, g();
    }
  }
