...
function pe(n, e, t, r) {
  ...
  for (let o = 0; o < 256; o++) {
    ...
    f[T + 0] = m ? 1 : 0, f[T + 1] = m ? 1 : 0, f[T + 2] = m ? 1 : 0, f[T + 3] = 1;
    const E = (2 * 256 + o) * 4;
    f[E + 0] = b, f[E + 1] = b, f[E + 2] = b, f[E + 3] = 1;
  }
...
class be {
  ...
  step(e, t) {
    ...
    const c = [this._width, this._height, 1], f = this._time, o = i, m = this._frame, b = t;
   ...
   */
  initRuntimePasses() {
    const e = this.gl, t = this.project, r = he(e), i = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const c of i) {
      const f = t.passes[c];
      if (!f) continue;
      const o = this.buildFragmentShader(f.glslSource);
      try {
        const m = le(e, ge, o), b = {
          program: m,
...
`);
  ...
  resolveChannelTexture(e, t) {
    switch (e.kind) {
      ...
      case "buffer": {
        const i = this._passes.find((f) => f.name === e.buffer);
        if (!i)
...
class ve {
  ...
  setupMouseTracking() {
    const e = (r) => {
      const i = this.canvas.getBoundingClientRect(), c = (r.clientX - i.left) * this.pixelRatio, f = (i.height - (r.clientY - i.top)) * this.pixelRatio;
      this.mouse[0] = c, this.mouse[1] = f;
    }, t = (r) => {
      const i = this.canvas.getBoundingClientRect(), c = (r.clientX - i.left) * this.pixelRatio, f = (i.height - (r.clientY - i.top)) * this.pixelRatio;
      this.mouse[2] = c, this.mouse[3] = f;
    };
  ...
  screenshot() {
    ...
    this.canvas.toBlob((c) => {
      ...
      const f = URL.createObjectURL(c), o = document.createElement("a");
      o.href = f, o.download = i, o.click(), URL.revokeObjectURL(f), console.log(`Screenshot saved: ${i}`);
    }, "image/png");
  }
  /**
...
`), f = 3, o = Math.max(0, i - f - 1), m = Math.min(c.length, i + f);
  ...
  extractCodeContextFromCommon(e) {
    ...
    if (!t) return null;
    const r = t.split(`
`), i = 3, c = Math.max(0, e - i - 1), f = Math.min(r.length, e + i);
    return r.slice(c, f).map((b, T) => {
...
(function(n) {
   ...
   */
  var t = function(r) {
    var i = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, c = 0, f = {}, o = {
      /**
      ...
      languages: {
        ...
        insertBefore: function(s, a, u, l) {
          ...
          var F = l[s];
          return l[s] = d, o.languages.DFS(o.languages, function(R, M) {
            M === F && R != s && (this[R] = d);
          }), d;
        },
        ...
        DFS: function s(a, u, l, h) {
          ...
          for (var v in a)
            if (a.hasOwnProperty(v)) {
              u.call(a, v, a[v], l || v);
              var p = a[v], F = o.util.type(p);
              F === "Object" && !h[d(p)] ? (h[d(p)] = !0, s(p, u, null, h)) : F === "Array" && !h[d(p)] && (h[d(p)] = !0, s(p, u, v, h));
            }
        }
      },
      plugins: {},
      ...
      highlightElement: function(s, a, u) {
        ...
        function F(M) {
          p.highlightedCode = M, o.hooks.run("before-insert", p), p.element.innerHTML = p.highlightedCode, o.hooks.run("after-highlight", p), o.hooks.run("complete", p), u && u.call(p.element);
        }
        if (o.hooks.run("before-sanity-check", p), d = p.element.parentElement, d && d.nodeName.toLowerCase() === "pre" && !d.hasAttribute("tabindex") && d.setAttribute("tabindex", "0"), !p.code) {
          o.hooks.run("complete", p), u && u.call(p.element);
          return;
        }
        if (o.hooks.run("before-highlight", p), !p.grammar) {
          F(o.util.encode(p.code));
          return;
        }
        if (a && r.Worker) {
          ...
          F(o.highlight(p.code, p.grammar, p.language));
      },
      /**
      ...
      for (var v in u)
        if (!(!u.hasOwnProperty(v) || !u[v])) {
          ...
          for (var F = 0; F < p.length; ++F) {
            if (d && d.cause == v + "," + F)
              return;
            var R = p[F], M = R.inside, D = !!R.lookbehind, V = !!R.greedy, ae = R.alias;
              ...
              if (!(P instanceof m)) {
                ...
                if (V) {
                  if (L = b(q, B, s, D), !L || L.index >= s.length)
                    break;
                  var I = L.index, ie = L.index + L[0].length, k = B;
  ...
  n.exports && (n.exports = t), typeof Z < "u" && (Z.Prism = t), t.languages.markup = {
    ...
    }, c = "✖ Error: File does not exist or is empty", f = {
      js: "javascript",
      py: "python",
      rb: "ruby",
      ps1: "powershell",
      ...
      if (w.matches(E)) {
        ...
        l && l.loadLanguages(a), x(
          ...
          function(h) {
            ...
            if (d) {
              var v = h.split(/\r\n?|\n/g), p = d[0], F = d[1] == null ? v.length : d[1];
              p < 0 && (p += v.length), p = Math.max(0, Math.min(p - 1, v.length)), F < 0 && (F += v.length), F = Math.max(0, Math.min(F, v.length)), h = v.slice(p, F).join(`
`), w.hasAttribute("data-start") || w.setAttribute("data-start", String(p + 1));
            }
      ...
      highlight: function(w) {
        for (var A = (w || document).querySelectorAll(E), s = 0, a; a = A[s++]; )
...
class _e {
  ...
  buildCodePanel() {
    ...
    f.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(o), f.innerHTML = b, f.classList.add("copied"), setTimeout(() => {
          f.innerHTML = m, f.classList.remove("copied");
        }, 1500);
      } catch (E) {
    ...
    }), this.codePanel.appendChild(i), this.codePanel.appendChild(f), this.codePanel.appendChild(c), e.length > 0 && T(0);
  }
}
class ye {
  constructor(e) {
  ...
  buildTabBar() {
    ...
    let c = "";
    const f = this.copyButton.innerHTML, o = `
      <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
        <path d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"/>
      </svg>
    `;
...
async function Fe(n, e, t, r) {
  const i = `/demos/${n}/shadertoy.config.json`, c = `/demos/${n}/config.json`, f = i in t ? i : c in t ? c : null;
  if (f) {
    const o = await t[f]();
...
async function Q(n, e, t) {
  ...
  const i = await e[r](), c = (t == null ? void 0 : t.layout) || "tabbed", f = (t == null ? void 0 : t.controls) ?? !0, o = ((m = t == null ? void 0 : t.meta) == null ? void 0 : m.title) || n.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" ");
  return {
    root: `/demos/${n}`,
    meta: {
      title: o,
...
async function Ce(n, e, t, r) {
  ...
  for (const d of f) {
    const v = e.passes[d];
    if (v)
      for (const p of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const F = (_ = v.channels) == null ? void 0 : _[p];
  ...
  for (const d of c) {
    ...
    const p = await r[v](), F = d.split("/").pop().replace(/\.[^.]+$/, "");
    o.push({
      name: F,
      source: p,
      filter: "linear",
...
const Ae = "course/day4/mandelbrot-waves-hook";
async function Re() {
  return Fe(Ae, /* @__PURE__ */ Object.assign({
    "/demos/course/day4/mandelbrot-waves-hook/bufferA.glsl": () => Promise.resolve().then(() => Le).then((r) => r.default),
    "/demos/course/day4/mandelbrot-waves-hook/common.glsl": () => Promise.resolve().then(() => Be).then((r) => r.default),
    "/demos/course/day4/mandelbrot-waves-hook/image.glsl": () => Promise.resolve().then(() => Pe).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day4/mandelbrot-waves-hook/config.json": () => Promise.resolve().then(() => De).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Ie(n) {
...
const Se = `void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    if (iFrame == 0) {
  ...
  __proto__: null,
  default: Me
}, Symbol.toStringTag, { value: "Module" })), ke = `void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    if (!inDomain(fragCoord, iResolution.xy)) {
