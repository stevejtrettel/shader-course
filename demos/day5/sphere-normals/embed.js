...
async function Ae(n, e, t, r) {
  ...
  if (s in t) {
    const i = await t[s]();
    return i.Image || i.BufferA || i.BufferB || i.BufferC || i.BufferD ? Se(n, i, e, r) : ee(n, e, i);
  } else
    return ee(n, e);
}
...
async function Se(n, e, t, r) {
  ...
  const w = e.title || n.split("-").map((E) => E.charAt(0).toUpperCase() + E.slice(1)).join(" "), x = e.author || null, C = e.description || null, R = e.layout || "tabbed", y = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Be = "course/day5/sphere-normals";
async function Le() {
  return Ae(Be, /* @__PURE__ */ Object.assign({
    "/demos/course/day5/sphere-normals/image.glsl": () => Promise.resolve().then(() => Ue).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day5/sphere-normals/config.json": () => Promise.resolve().then(() => $e).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Oe(n) {
    ...
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, l = 0, i = {}, a = {
      ...
      languages: {
        ...
        insertBefore: function(c, o, u, d) {
          ...
          var F = d[c];
          return d[c] = m, a.languages.DFS(a.languages, function(A, k) {
            k === F && A != c && (this[A] = m);
          }), m;
        },
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var F = 0; F < b.length; ++F) {
            if (m && m.cause == _ + "," + F)
              return;
            var A = b[F], k = A.inside, V = !!A.lookbehind, q = !!A.greedy, ce = A.alias;
            if (q && !A.pattern.global) {
              var le = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, le + "g");
            }
            for (var K = A.pattern || A, S = d.next, L = p; S !== o.tail && !(m && L >= m.reach); L += S.value.length, S = S.next) {
              var P = S.value;
              ...
              if (!(P instanceof f)) {
                ...
                if (S = x(o, j, de), W && x(o, S, W), U > 1) {
                  ...
                  v(c, o, u, S.prev, L, G), m && G.reach > m.reach && (m.reach = G.reach);
                }
              }
            }
          }
