...
var h = (n, e, t) => pe(n, typeof e != "symbol" ? e + "" : e, t);
function Y(n, e, t) {
...
function J(n, e) {
  ...
  if (r !== n.FRAMEBUFFER_COMPLETE)
    throw n.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${Ee(n, r)}`);
...
`), i = 3, a = Math.max(0, s - i - 1), m = Math.min(l.length, s + i);
    return l.slice(a, m).map((w, x) => {
      const C = a + x + 1, A = C === s, y = String(C).padStart(4, " "), E = this.escapeHTML(w);
...
function ne(n) {
  return typeof n == "string" ? Ae(n) ? { buffer: n } : n === "keyboard" ? { keyboard: !0 } : { texture: n } : n;
}
async function Se(n, e, t, r) {
  const s = `/demos/${n}/config.json`;
  if (s in t) {
    const i = await t[s]();
    return i.Image || i.BufferA || i.BufferB || i.BufferC || i.BufferD ? Re(n, i, e, r) : ee(n, e, i);
  } else
    return ee(n, e);
}
...
async function Re(n, e, t, r) {
  ...
  for (const E of i) {
    ...
    const o = await r[c](), u = E.split("/").pop(), d = u.replace(/\.[^.]+$/, "");
    m.push({
      name: d,
      filename: u,
...
const Be = "course/day5/sdf-circle-2d";
async function Le() {
  return Se(Be, /* @__PURE__ */ Object.assign({
    "/demos/course/day5/sdf-circle-2d/image.glsl": () => Promise.resolve().then(() => Ue).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({
    "/demos/course/day5/sdf-circle-2d/config.json": () => Promise.resolve().then(() => $e).then((r) => r.default)
  }), /* @__PURE__ */ Object.assign({}));
}
async function Oe(n) {
  ...
  const t = await Le(), r = Fe(t.layout, {
    container: e,
    project: t
  }), s = new Te({
              ...
              if (!(P instanceof m)) {
                ...
                if (K) {
                  ...
                  if (M -= R.value.length, L = M, R.value instanceof m)
                    continue;
                  for (var D = R; D !== o.tail && (M < ue || typeof D.value == "string"); D = D.next)
                    U++, M += D.value.length;
                ...
                var de = new m(_, k ? a.tokenize($, k) : $, ce, $);
                if (R = x(o, j, de), W && x(o, R, W), U > 1) {
                  var G = {
                    cause: _ + "," + F,
                    reach: X
                  };
...
function De(n, e, t) {
  ...
  function g() {
    const x = i.value;
    m.textContent = x + `
`, Pe.highlightElement(m);
    const C = x.split(`
`);
    s.innerHTML = C.map((A, y) => `<span>${y + 1}</span>`).join(""), t && t(x);
  }
  function v() {
  ...
  function w(x) {
    if (x.key === "Tab") {
      x.preventDefault();
      const C = i.selectionStart, A = i.selectionEnd, y = i.value;
      i.value = y.substring(0, C) + "  " + y.substring(A), i.selectionStart = i.selectionEnd = C + 2, g();
    }
  }
...
const ie = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createEditor: De
}, Symbol.toStringTag, { value: "Module" })), Ie = `float sdCircle(vec2 p, float r) {
    return length(p) - r;
}
