    ...
    for (const o of s) {
      ...
      try {
        ...
        if (g && this.project.commonSource) {
          const A = parseInt(g[1], 10), x = v.boilerplateLinesBeforeCommon + 2, F = x + v.commonLineCount - 1;
          A >= x && A <= F && (T = !0, _ = A - x + 1);
        }
...
`, "");
      let x = A;
      return T && _ !== null && (x = A.replace(/Line \d+:/, `Line ${_}:`), x = x.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${_}:`)), {
...
`), p = 3, c = Math.max(0, s - p - 1), E = Math.min(o.length, s + p);
    return o.slice(c, E).map((g, T) => {
      const _ = c + T + 1, A = _ === s, x = String(_).padStart(4, " "), F = this.escapeHTML(g);
      return A ? `<span class="error-line-highlight">${x} │ ${F}</span>` : `<span class="context-line">${x} │ ${F}</span>`;
    }).join("");
  }
  /**
...
`), s = 3, o = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(o, p).map((b, v) => {
      const g = o + v + 1, T = g === e, _ = String(g).padStart(4, " "), A = this.escapeHTML(b);
      return T ? `<span class="error-line-highlight">${_} │ ${A}</span>` : `<span class="context-line">${_} │ ${A}</span>`;
    }).join("");
  }
  /**
  ...
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, o = 0, p = {}, c = {
      ...
      tokenize: function(i, a) {
        ...
        var d = new g();
        return T(d, d.head, i), v(i, d, a, d.head, 0), A(d);
      },
      /**
       * @namespace
       * @memberof Prism
...
class ve {
  ...
  buildCodePanel() {
    ...
    `, v = (g) => {
      const T = e[g];
      c = T.source;
      const _ = document.createElement("pre"), A = document.createElement("code");
      A.className = "language-cpp", A.textContent = T.source, _.appendChild(A), o.innerHTML = "", o.appendChild(_), te.highlightElement(A);
    };
    p.addEventListener("click", async () => {
      ...
      _.className = "tab-button", _.textContent = g.name, T === 0 && _.classList.add("active"), _.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((A) => A.classList.remove("active")), _.classList.add("active"), v(T);
      }), s.appendChild(_);
...
`);
        ...
        x.className = "language-cpp", x.textContent = o, A.appendChild(x), T.appendChild(_), T.appendChild(A), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(T), te.highlightElement(x);
      }
    };
...
async function we(n, e, t, r) {
  ...
  for (const h of E) {
    ...
    if (w)
      for (const C of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const S = (R = w.channels) == null ? void 0 : R[C];
  ...
  const T = ((d = o.meta) == null ? void 0 : d.title) || n.split("-").map((h) => h.charAt(0).toUpperCase() + h.slice(1)).join(" "), _ = ((m = o.meta) == null ? void 0 : m.author) || null, A = ((y = o.meta) == null ? void 0 : y.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
    meta: { title: T, author: _, description: A },
    layout: x,
    controls: F,
    commonSource: p,
    passes: g,
...
const Fe = "course/day1/red-pulsing";
async function Ae() {
  return _e(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day1/red-pulsing/image.glsl": () => Promise.resolve().then(() => Ce).then((r) => r.default)
