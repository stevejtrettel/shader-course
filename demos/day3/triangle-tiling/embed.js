...
function Z(n, e) {
  ...
  if (r !== n.FRAMEBUFFER_COMPLETE)
    throw n.deleteFramebuffer(t), new Error(`Framebuffer incomplete: ${de(n, r)}`);
  ...
  initRuntimePasses() {
    ...
    for (const o of s) {
      ...
      try {
        ...
        }, v = H(e, this._width, this._height), g = H(e, this._width, this._height), T = Z(e, v), y = {
          name: o,
        ...
        if (g && this.project.commonSource) {
          const C = parseInt(g[1], 10), x = v.boilerplateLinesBeforeCommon + 2, F = x + v.commonLineCount - 1;
          C >= x && C <= F && (T = !0, y = C - x + 1);
        }
...
class ge {
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((b) => b.isFromCommon), r = e.filter((b) => !b.isFromCommon), c = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: b, error: v, source: g, isFromCommon: T, originalLine: y }) => {
      const C = v.replace(`Shader compilation failed:
`, "");
      let x = C;
...
`), s = 3, o = Math.max(0, e - s - 1), p = Math.min(r.length, e + s);
    return r.slice(o, p).map((b, v) => {
      const g = o + v + 1, T = g === e, y = String(g).padStart(4, " "), C = this.escapeHTML(b);
  ...
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, o = 0, p = {}, c = {
      ...
      util: {
        ...
        clone: function i(a, u) {
          ...
          switch (c.util.type(a)) {
            ...
            case "Array":
              return h = c.util.objId(a), u[h] ? u[h] : (l = [], u[h] = l, /** @type {Array} */
              /** @type {any} */
              a.forEach(function(_, d) {
                l[d] = i(_, u);
              }), /** @type {any} */
              l);
            default:
        ...
        insertBefore: function(i, a, u, l) {
          ...
          for (var _ in h)
            if (h.hasOwnProperty(_)) {
              if (_ == a)
                for (var d in u)
                  u.hasOwnProperty(d) && (m[d] = u[d]);
              u.hasOwnProperty(_) || (m[_] = h[_]);
            }
          var w = l[i];
        ...
        DFS: function i(a, u, l, h) {
          ...
          for (var _ in a)
            if (a.hasOwnProperty(_)) {
              u.call(a, _, a[_], l || _);
              var d = a[_], w = c.util.type(d);
              w === "Object" && !h[m(d)] ? (h[m(d)] = !0, i(d, u, null, h)) : w === "Array" && !h[m(d)] && (h[m(d)] = !0, i(d, u, _, h));
            }
        }
      },
      plugins: {},
      ...
      highlightElement: function(i, a, u) {
        ...
        function w(S) {
          d.highlightedCode = S, c.hooks.run("before-insert", d), d.element.innerHTML = d.highlightedCode, c.hooks.run("after-highlight", d), c.hooks.run("complete", d), u && u.call(d.element);
        }
        if (c.hooks.run("before-sanity-check", d), m = d.element.parentElement, m && m.nodeName.toLowerCase() === "pre" && !m.hasAttribute("tabindex") && m.setAttribute("tabindex", "0"), !d.code) {
          c.hooks.run("complete", d), u && u.call(d.element);
          return;
        }
        if (c.hooks.run("before-highlight", d), !d.grammar) {
          w(c.util.encode(d.code));
          return;
        }
        if (a && r.Worker) {
          ...
          w(c.highlight(d.code, d.grammar, d.language));
      },
      /**
      ...
      tokenize: function(i, a) {
        ...
        var h = new g();
        return T(h, h.head, i), v(i, h, a, h.head, 0), C(h);
      },
      /**
       * @namespace
       * @memberof Prism
    ...
    E.stringify = function i(a, u) {
      ...
      m && (Array.isArray(m) ? Array.prototype.push.apply(h.classes, m) : h.classes.push(m)), c.hooks.run("wrap", h);
      var _ = "";
      for (var d in h.attributes)
        _ += " " + d + '="' + (h.attributes[d] || "").replace(/"/g, "&quot;") + '"';
      return "<" + h.tag + ' class="' + h.classes.join(" ") + '"' + _ + ">" + h.content + "</" + h.tag + ">";
    };
    function b(i, a, u, l) {
      i.lastIndex = a;
      var h = i.exec(u);
      if (h && l && h[1]) {
        var m = h[1].length;
        h.index += m, h[0] = h[0].slice(m);
      }
      return h;
    }
    function v(i, a, u, l, h, m) {
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var w = 0; w < d.length; ++w) {
            if (m && m.cause == _ + "," + w)
              return;
            var A = d[w], S = A.inside, P = !!A.lookbehind, I = !!A.greedy, X = A.alias;
            if (I && !A.pattern.global) {
              var ne = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, ne + "g");
            }
            for (var W = A.pattern || A, L = l.next, B = h; L !== a.tail && !(m && B >= m.reach); B += L.value.length, L = L.next) {
              var U = L.value;
              ...
              if (!(U instanceof E)) {
                ...
                var j = L.prev;
                G && (j = T(a, j, G), B += G.length), y(a, j, $);
                var ae = new E(_, S ? c.tokenize(N, S) : N, X, N);
                if (L = T(a, j, ae), K && T(a, L, K), $ > 1) {
                  var q = {
                    cause: _ + "," + w,
                    reach: V
                  };
    ...
    if (!r.document)
      return r.addEventListener && (c.disableWorkerMessageHandler || r.addEventListener("message", function(i) {
        ...
        r.postMessage(c.highlight(l, c.languages[u], u)), h && r.close();
      }, !1)), c;
      ...
      if (F.matches(g)) {
        ...
        l && l.loadLanguages(a), T(
          ...
          function(h) {
            ...
            if (m) {
              var _ = h.split(/\r\n?|\n/g), d = m[0], w = m[1] == null ? _.length : m[1];
              d < 0 && (d += _.length), d = Math.max(0, Math.min(d - 1, _.length)), w < 0 && (w += _.length), w = Math.max(0, Math.min(w, _.length)), h = _.slice(d, w).join(`
`), F.hasAttribute("data-start") || F.setAttribute("data-start", String(d + 1));
            }
            R.textContent = h, t.highlightElement(R);
          },
          function(h) {
            F.setAttribute(c, v), R.textContent = h;
          }
        );
      }
  ...
  buildCodePanel() {
    ...
    p.addEventListener("click", async () => {
      ...
      y.className = "tab-button", y.textContent = g.name, T === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((C) => C.classList.remove("active")), y.classList.add("active"), v(T);
      }), s.appendChild(y);
...
async function we(n, e, t, r) {
  ...
  for (const d of c) {
    const w = `/demos/${n}/${d.replace(/^\.\//, "")}`;
    if (!(w in r))
      throw new Error(`Texture not found: ${d} (expected at ${w})`);
    const A = await r[w](), S = d.split("/").pop().replace(/\.[^.]+$/, "");
    b.push({
      name: S,
      source: A,
      filter: "linear",
  ...
  for (const d of E) {
    ...
    }, S = w.source || A[d], P = `/demos/${n}/${S}`;
    if (!(P in t))
      throw new Error(`Missing shader file: ${P}`);
    const I = await t[P](), X = [
      z((i = w.channels) == null ? void 0 : i.iChannel0, v),
      z((a = w.channels) == null ? void 0 : a.iChannel1, v),
      z((u = w.channels) == null ? void 0 : u.iChannel2, v),
      z((l = w.channels) == null ? void 0 : l.iChannel3, v)
    ];
    g[d] = {
      name: d,
      glslSource: I,
  ...
  const T = ((h = o.meta) == null ? void 0 : h.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), y = ((m = o.meta) == null ? void 0 : m.author) || null, C = ((_ = o.meta) == null ? void 0 : _.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Fe = "course/day3/triangle-tiling";
async function Ce() {
  return ye(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/triangle-tiling/image.glsl": () => Promise.resolve().then(() => Ae).then((r) => r.default)
