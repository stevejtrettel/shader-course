    ...
    for (const c of i) {
      ...
      try {
        ...
        if (E && this.project.commonSource) {
          const A = parseInt(E[1], 10), y = T.boilerplateLinesBeforeCommon + 2, w = y + T.commonLineCount - 1;
          A >= y && A <= w && (x = !0, _ = A - y + 1);
        }
...
`, "");
      let y = A;
      return x && _ !== null && (y = A.replace(/Line \d+:/, `Line ${_}:`), y = y.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${_}:`)), {
...
`), p = 3, o = Math.max(0, i - p - 1), m = Math.min(c.length, i + p);
    return c.slice(o, m).map((E, x) => {
      const _ = o + x + 1, A = _ === i, y = String(_).padStart(4, " "), w = this.escapeHTML(E);
      return A ? `<span class="error-line-highlight">${y} │ ${w}</span>` : `<span class="context-line">${y} │ ${w}</span>`;
    }).join("");
  }
  /**
...
`), i = 3, c = Math.max(0, e - i - 1), p = Math.min(r.length, e + i);
    return r.slice(c, p).map((b, T) => {
      const E = c + T + 1, x = E === e, _ = String(E).padStart(4, " "), A = this.escapeHTML(b);
      return x ? `<span class="error-line-highlight">${_} │ ${A}</span>` : `<span class="context-line">${_} │ ${A}</span>`;
    }).join("");
  }
  /**
...
class Te {
  ...
  buildCodePanel() {
    ...
    `, T = (E) => {
      ...
      A.className = "language-cpp", A.textContent = x.source, _.appendChild(A), c.innerHTML = "", c.appendChild(_), te.highlightElement(A);
    };
    p.addEventListener("click", async () => {
      ...
      _.className = "tab-button", _.textContent = E.name, x === 0 && _.classList.add("active"), _.addEventListener("click", () => {
        i.querySelectorAll(".tab-button").forEach((A) => A.classList.remove("active")), _.classList.add("active"), T(x);
      }), i.appendChild(_);
...
`);
        ...
        y.className = "language-cpp", y.textContent = c, A.appendChild(y), x.appendChild(_), x.appendChild(A), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(x), te.highlightElement(y);
      }
    };
...
async function we(n, e, t, r) {
  ...
  const T = ((u = e.meta) == null ? void 0 : u.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), E = ((l = e.meta) == null ? void 0 : l.author) || null, x = ((h = e.meta) == null ? void 0 : h.description) || null, _ = e.layout || "tabbed", A = e.controls ?? !0;
  return {
    root: `/demos/${n}`,
