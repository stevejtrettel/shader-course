  ...
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, o = 0, p = {}, c = {
        ...
        DFS: function i(a, u, l, h) {
          ...
          for (var y in a)
            if (a.hasOwnProperty(y)) {
              u.call(a, y, a[y], l || y);
              var d = a[y], w = c.util.type(d);
              w === "Object" && !h[m(d)] ? (h[m(d)] = !0, i(d, u, null, h)) : w === "Array" && !h[m(d)] && (h[m(d)] = !0, i(d, u, y, h));
            }
        }
      },
      plugins: {},
      ...
      tokenize: function(i, a) {
        ...
        var h = new g();
        return T(h, h.head, i), v(i, h, a, h.head, 0), R(h);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var y in u)
        if (!(!u.hasOwnProperty(y) || !u[y])) {
          ...
          for (var w = 0; w < d.length; ++w) {
            if (m && m.cause == y + "," + w)
              return;
            var C = d[w], S = C.inside, P = !!C.lookbehind, $ = !!C.greedy, X = C.alias;
            if ($ && !C.pattern.global) {
              var ne = C.pattern.toString().match(/[imsuy]*$/)[0];
              C.pattern = RegExp(C.pattern.source, ne + "g");
            }
            for (var W = C.pattern || C, L = l.next, B = h; L !== a.tail && !(m && B >= m.reach); B += L.value.length, L = L.next) {
              var U = L.value;
      ...
      if (F.matches(g)) {
        ...
        l && l.loadLanguages(a), T(
          ...
          function(h) {
            ...
            if (m) {
              var y = h.split(/\r\n?|\n/g), d = m[0], w = m[1] == null ? y.length : m[1];
              d < 0 && (d += y.length), d = Math.max(0, Math.min(d - 1, y.length)), w < 0 && (w += y.length), w = Math.max(0, Math.min(w, y.length)), h = y.slice(d, w).join(`
`), F.hasAttribute("data-start") || F.setAttribute("data-start", String(d + 1));
            }
            A.textContent = h, t.highlightElement(A);
          },
          function(h) {
            F.setAttribute(c, v), A.textContent = h;
          }
        );
      }
...
async function we(n, e, t, r) {
  ...
  for (const d of c) {
    ...
    const C = await r[w](), S = d.split("/").pop().replace(/\.[^.]+$/, "");
    b.push({
      name: S,
      source: C,
      filter: "linear",
  ...
  const T = ((h = o.meta) == null ? void 0 : h.title) || n.split("-").map((d) => d.charAt(0).toUpperCase() + d.slice(1)).join(" "), _ = ((m = o.meta) == null ? void 0 : m.author) || null, R = ((y = o.meta) == null ? void 0 : y.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
...
const Fe = "course/day3/halfspace-single";
async function Re() {
  return _e(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/halfspace-single/image.glsl": () => Promise.resolve().then(() => Ce).then((r) => r.default)
