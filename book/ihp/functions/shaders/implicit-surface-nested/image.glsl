...
vec3 render(vec3 ro, vec3 rd) {
    ...
    // March through box, accumulating surface layers
    ...
    // Composite surfaces over background
    ...
    if (depth > 0.001) {
        ...
        grad -= n*dot(grad, n);
        for (int k = 0; k < NUM_LEVELS; k++) {
            float w = abs(val - getLevel(k)) / length(grad);
            float la = (HIGHLIGHT < 0 || k == HIGHLIGHT) ? 1.0 : GHOST_OPACITY;
            col = mix(col, levelColor(k)*0.1,
                      la * mix(1.0, 0.0, clamp(w - ISO_LINE_WIDTH*tnf.x, 0.0, 1.0)));
        }
        if (tnf.y < lastHitT + 0.01) {
            p = ro + rd*(tnf.y - 0.01);
            val = fWorld(p);
            grad = fGrad(p);
            grad -= n2*dot(grad, n2);
            for (int k = 0; k < NUM_LEVELS; k++) {
                ...
                float la = (HIGHLIGHT < 0 || k == HIGHLIGHT) ? 1.0 : GHOST_OPACITY;
                col = mix(col, levelColor(k)*0.1,
                          la * mix(1.0, 0.0, clamp(w - ISO_LINE_WIDTH*tnf.y, 0.0, 1.0)));
            }
        }
    }
#endif
