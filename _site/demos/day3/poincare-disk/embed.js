...
function de(n, e, t, r) {
  ...
  for (let c = 0; c < 256; c++) {
    ...
    f[v + 0] = E ? 1 : 0, f[v + 1] = E ? 1 : 0, f[v + 2] = E ? 1 : 0, f[v + 3] = 1;
    const g = (2 * 256 + c) * 4;
    f[g + 0] = b, f[g + 1] = b, f[g + 2] = b, f[g + 3] = 1;
  }
...
class me {
  ...
  step(e, t) {
    ...
    const o = [this._width, this._height, 1], f = this._time, c = s, E = this._frame, b = t;
  ...
  initRuntimePasses() {
    ...
    for (const o of s) {
      ...
      try {
        ...
        }, v = j(e, this._width, this._height), g = j(e, this._width, this._height), T = Z(e, v), y = {
          name: o,
...
`, "");
      let x = R;
      return T && y !== null && (x = R.replace(/Line \d+:/, `Line ${y}:`), x = x.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${y}:`)), {
...
`), f = 3, c = Math.max(0, s - f - 1), E = Math.min(o.length, s + f);
    return o.slice(c, E).map((g, T) => {
      const y = c + T + 1, R = y === s, x = String(y).padStart(4, " "), F = this.escapeHTML(g);
...
`), s = 3, o = Math.max(0, e - s - 1), f = Math.min(r.length, e + s);
    return r.slice(o, f).map((b, v) => {
      const g = o + v + 1, T = g === e, y = String(g).padStart(4, " "), R = this.escapeHTML(b);
      return T ? `<span class="error-line-highlight">${y} │ ${R}</span>` : `<span class="context-line">${y} │ ${R}</span>`;
    }).join("");
  }
  /**
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var w = 0; w < h.length; ++w) {
            if (m && m.cause == _ + "," + w)
              return;
            var C = h[w], S = C.inside, P = !!C.lookbehind, I = !!C.greedy, X = C.alias;
...
class ve {
  ...
  buildCodePanel() {
    ...
    `, v = (g) => {
      ...
      R.className = "language-cpp", R.textContent = T.source, y.appendChild(R), o.innerHTML = "", o.appendChild(y), te.highlightElement(R);
    };
    f.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(c), f.innerHTML = b, f.classList.add("copied"), setTimeout(() => {
      ...
      y.className = "tab-button", y.textContent = g.name, T === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((R) => R.classList.remove("active")), y.classList.add("active"), v(T);
      }), s.appendChild(y);
    }), this.codePanel.appendChild(s), this.codePanel.appendChild(f), this.codePanel.appendChild(o), e.length > 0 && v(0);
  }
}
class Te {
  constructor(e) {
...
`), T = document.createElement("pre"), y = document.createElement("div");
        y.className = "tabbed-line-numbers", y.innerHTML = g.map((F, A) => A + 1).join(`
`);
        const R = document.createElement("div");
        R.className = "tabbed-code-content";
        const x = document.createElement("code");
        x.className = "language-cpp", x.textContent = o, R.appendChild(x), T.appendChild(y), T.appendChild(R), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(T), te.highlightElement(x);
      }
    };
...
async function we(n, e, t, r) {
  ...
  for (const h of E) {
    ...
    const I = await t[P](), X = [
      H((i = w.channels) == null ? void 0 : i.iChannel0, v),
      H((a = w.channels) == null ? void 0 : a.iChannel1, v),
      H((u = w.channels) == null ? void 0 : u.iChannel2, v),
      H((l = w.channels) == null ? void 0 : l.iChannel3, v)
    ];
    g[h] = {
      name: h,
      glslSource: I,
  ...
  const T = ((d = o.meta) == null ? void 0 : d.title) || n.split("-").map((h) => h.charAt(0).toUpperCase() + h.slice(1)).join(" "), y = ((m = o.meta) == null ? void 0 : m.author) || null, R = ((_ = o.meta) == null ? void 0 : _.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
    meta: { title: T, author: y, description: R },
    layout: x,
    controls: F,
    commonSource: f,
    passes: g,
...
const Fe = "course/day3/poincare-disk";
async function Re() {
  return ye(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/poincare-disk/image.glsl": () => Promise.resolve().then(() => Ce).then((r) => r.default)
