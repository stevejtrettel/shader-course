...
var p = (n, e, t) => se(n, typeof e != "symbol" ? e + "" : e, t);
function Y(n, e, t) {
...
function de(n, e, t, r) {
  const f = new Float32Array(3072);
  for (let c = 0; c < 256; c++) {
    const E = t.get(c) || !1, v = r.get(c) || 0, b = (0 * 256 + c) * 4;
    f[b + 0] = E ? 1 : 0, f[b + 1] = E ? 1 : 0, f[b + 2] = E ? 1 : 0, f[b + 3] = 1;
    const g = (2 * 256 + c) * 4;
    f[g + 0] = v, f[g + 1] = v, f[g + 2] = v, f[g + 3] = 1;
  }
...
class me {
  constructor(e) {
    ...
    p(this, "_frame", 0);
    p(this, "_time", 0);
    p(this, "_lastStepTime", null);
    p(this, "_passes", []);
    p(this, "_textures", []);
    p(this, "_keyboardTexture", null);
    p(this, "_blackTexture", null);
    // Keyboard state tracking (Maps keycodes to state)
    p(this, "_keyStates", /* @__PURE__ */ new Map());
    // true = down, false = up
    p(this, "_toggleStates", /* @__PURE__ */ new Map());
    // 0.0 or 1.0
    // Compilation errors (if any occurred during initialization)
    p(this, "_compilationErrors", []);
  ...
  step(e, t) {
    ...
    const o = [this._width, this._height, 1], f = this._time, c = s, E = this._frame, v = t;
    r.viewport(0, 0, this._width, this._height);
    const b = ["BufferA", "BufferB", "BufferC", "BufferD", "Image"];
    for (const g of b) {
      const T = this._passes.find((y) => y.name === g);
  ...
  resize(e, t) {
    ...
    for (const s of this._passes)
      r.deleteTexture(s.currentTexture), r.deleteTexture(s.previousTexture), r.deleteFramebuffer(s.framebuffer), s.currentTexture = j(r, e, t), s.previousTexture = j(r, e, t), s.framebuffer = Z(r, s.currentTexture);
  }
  /**
  ...
  initProjectTextures() {
    ...
    for (const t of this.project.textures) {
      ...
      o.crossOrigin = "anonymous", o.onload = () => {
        ...
        const f = t.filter === "nearest" ? e.NEAREST : e.LINEAR;
        e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, f), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MAG_FILTER, f);
  ...
  initRuntimePasses() {
    ...
    for (const o of s) {
      const f = t.passes[o];
      if (!f) continue;
      const c = this.buildFragmentShader(f.glslSource);
      try {
        const E = oe(e, fe, c), v = {
          program: E,
        ...
        }, b = j(e, this._width, this._height), g = j(e, this._width, this._height), T = Z(e, b), y = {
          name: o,
          projectChannels: f.channels,
          vao: r,
          uniforms: v,
          framebuffer: T,
          currentTexture: b,
        ...
        const v = E instanceof Error ? E.message : String(E), b = this.getLineMapping(), g = v.match(/ERROR:\s*\d+:(\d+):/);
        let T = !1, y = null;
        if (g && this.project.commonSource) {
          const C = parseInt(g[1], 10), x = b.boilerplateLinesBeforeCommon + 2, F = x + b.commonLineCount - 1;
          C >= x && C <= F && (T = !0, y = C - x + 1);
        }
        this._compilationErrors.push({
          ...
          isFromCommon: T,
          originalLine: y
        }), console.error(`Failed to compile ${o}:`, v);
      }
    }
  }
  /**
   ...
   */
  getLineMapping() {
    const e = pe + 1, t = this.project.commonSource ? this.project.commonSource.split(`
`).length : 0;
...
`);
  ...
  resolveChannelTexture(e, t) {
    switch (e.kind) {
      ...
      case "buffer": {
        const s = this._passes.find((f) => f.name === e.buffer);
        if (!s)
...
class ge {
  constructor(e) {
    ...
    p(this, "currentFps", 0);
    // Playback controls
    p(this, "controlsContainer", null);
    p(this, "playPauseButton", null);
    p(this, "isPaused", !1);
    // Error overlay
    p(this, "errorOverlay", null);
    // Resize observer
  ...
  setupMouseTracking() {
    const e = (r) => {
      const s = this.canvas.getBoundingClientRect(), o = (r.clientX - s.left) * this.pixelRatio, f = (s.height - (r.clientY - s.top)) * this.pixelRatio;
      this.mouse[0] = o, this.mouse[1] = f;
    }, t = (r) => {
      const s = this.canvas.getBoundingClientRect(), o = (r.clientX - s.left) * this.pixelRatio, f = (s.height - (r.clientY - s.top)) * this.pixelRatio;
      this.mouse[2] = o, this.mouse[3] = f;
    };
  ...
  screenshot() {
    ...
    this.canvas.toBlob((o) => {
      ...
      const f = URL.createObjectURL(o), c = document.createElement("a");
      c.href = f, c.download = s, c.click(), URL.revokeObjectURL(f), console.log(`Screenshot saved: ${s}`);
    }, "image/png");
  }
  /**
  ...
  showErrorOverlay(e) {
    ...
    const t = e.filter((v) => v.isFromCommon), r = e.filter((v) => !v.isFromCommon), c = [...t.length > 0 ? [t[0]] : [], ...r].map(({ passName: v, error: b, source: g, isFromCommon: T, originalLine: y }) => {
      const C = b.replace(`Shader compilation failed:
`, "");
      let x = C;
      return T && y !== null && (x = C.replace(/Line \d+:/, `Line ${y}:`), x = x.replace(/ERROR:\s*\d+:(\d+):/, `ERROR: 0:${y}:`)), {
        passName: T ? "common.glsl" : v,
        error: this.parseShaderError(x),
        codeContext: T ? this.extractCodeContextFromCommon(y) : this.extractCodeContext(x, g)
      };
    }).map(({ passName: v, error: b, codeContext: g }) => `
      <div class="error-section">
        <div class="error-pass-name">${v}</div>
        <pre class="error-content">${this.escapeHTML(b)}</pre>
...
`), f = 3, c = Math.max(0, s - f - 1), E = Math.min(o.length, s + f);
    return o.slice(c, E).map((g, T) => {
      const y = c + T + 1, C = y === s, x = String(y).padStart(4, " "), F = this.escapeHTML(g);
      return C ? `<span class="error-line-highlight">${x} │ ${F}</span>` : `<span class="context-line">${x} │ ${F}</span>`;
    }).join("");
  }
  /**
  ...
  extractCodeContextFromCommon(e) {
    ...
    if (!t) return null;
    const r = t.split(`
`), s = 3, o = Math.max(0, e - s - 1), f = Math.min(r.length, e + s);
    return r.slice(o, f).map((v, b) => {
      const g = o + b + 1, T = g === e, y = String(g).padStart(4, " "), C = this.escapeHTML(v);
      return T ? `<span class="error-line-highlight">${y} │ ${C}</span>` : `<span class="context-line">${y} │ ${C}</span>`;
    }).join("");
  }
  /**
...
(function(n) {
   ...
   */
  var t = function(r) {
    var s = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, o = 0, f = {}, c = {
      /**
        ...
        insertBefore: function(i, a, u, l) {
          ...
          for (var _ in d)
            if (d.hasOwnProperty(_)) {
              ...
              u.hasOwnProperty(_) || (m[_] = d[_]);
            }
          var w = l[i];
          return l[i] = m, c.languages.DFS(c.languages, function(A, S) {
            S === w && A != i && (this[A] = m);
          }), m;
        },
        ...
        DFS: function i(a, u, l, d) {
          ...
          for (var _ in a)
            if (a.hasOwnProperty(_)) {
              u.call(a, _, a[_], l || _);
              var h = a[_], w = c.util.type(h);
              w === "Object" && !d[m(h)] ? (d[m(h)] = !0, i(h, u, null, d)) : w === "Array" && !d[m(h)] && (d[m(h)] = !0, i(h, u, _, d));
            }
        }
      },
      plugins: {},
      ...
      tokenize: function(i, a) {
        ...
        var d = new g();
        return T(d, d.head, i), b(i, d, a, d.head, 0), C(d);
      },
      /**
       * @namespace
       * @memberof Prism
      ...
      for (var _ in u)
        if (!(!u.hasOwnProperty(_) || !u[_])) {
          ...
          for (var w = 0; w < h.length; ++w) {
            if (m && m.cause == _ + "," + w)
              return;
            var A = h[w], S = A.inside, P = !!A.lookbehind, I = !!A.greedy, X = A.alias;
            if (I && !A.pattern.global) {
              var ne = A.pattern.toString().match(/[imsuy]*$/)[0];
              A.pattern = RegExp(A.pattern.source, ne + "g");
            }
            for (var W = A.pattern || A, L = l.next, B = d; L !== a.tail && !(m && B >= m.reach); B += L.value.length, L = L.next) {
              var U = L.value;
              ...
              if (!(U instanceof E)) {
                var $ = 1, M;
                if (I) {
                  if (M = v(W, B, i, P), !M || M.index >= i.length)
                    break;
                  ...
                  for (var D = L; D !== a.tail && (k < re || typeof D.value == "string"); D = D.next)
                    $++, k += D.value.length;
                  $--, U = i.slice(B, k), M.index -= B;
                } else if (M = v(W, 0, U, P), !M)
                  continue;
                var O = M.index, z = M[0], G = U.slice(0, O), K = U.slice(O + z.length), V = B + U.length;
                m && V > m.reach && (m.reach = V);
                var N = L.prev;
                G && (N = T(a, N, G), B += G.length), y(a, N, $);
                var ae = new E(_, S ? c.tokenize(z, S) : z, X, z);
                if (L = T(a, N, ae), K && T(a, L, K), $ > 1) {
                  var q = {
                    cause: _ + "," + w,
                    reach: V
                  };
                  b(i, a, u, L.prev, B, q), m && q.reach > m.reach && (m.reach = q.reach);
                }
              }
            }
          }
    ...
    if (!c.manual) {
      ...
      R === "loading" || R === "interactive" && x && x.defer ? document.addEventListener("DOMContentLoaded", F) : window.requestAnimationFrame ? window.requestAnimationFrame(F) : window.setTimeout(F, 16);
    }
    return c;
  }(e);
  n.exports && (n.exports = t), typeof Q < "u" && (Q.Prism = t), t.languages.markup = {
    ...
    var r = "Loading…", s = function(x, F) {
      ...
      h: "c",
      tex: "latex"
    }, c = "data-src-status", E = "loading", v = "loaded", b = "failed", g = "pre[data-src]:not([" + c + '="' + v + '"]):not([' + c + '="' + E + '"])';
    function T(x, F, R) {
      var i = new XMLHttpRequest();
      i.open("GET", x, !0), i.onreadystatechange = function() {
        i.readyState == 4 && (i.status < 400 && i.responseText ? F(i.responseText) : i.status >= 400 ? R(s(i.status, i.statusText)) : R(o));
      }, i.send(null);
    }
    function y(x) {
      var F = /^\s*(\d+)\s*(?:(,)\s*(?:(\d+)\s*)?)?$/.exec(x || "");
      if (F) {
        var R = Number(F[1]), i = F[2], a = F[3];
    ...
    t.hooks.add("before-highlightall", function(x) {
      ...
      if (F.matches(g)) {
        ...
        t.util.setLanguage(R, a), t.util.setLanguage(F, a);
        var l = t.plugins.autoloader;
        l && l.loadLanguages(a), T(
          i,
          function(d) {
            F.setAttribute(c, v);
            var m = y(F.getAttribute("data-range"));
            if (m) {
              var _ = d.split(/\r\n?|\n/g), h = m[0], w = m[1] == null ? _.length : m[1];
              h < 0 && (h += _.length), h = Math.max(0, Math.min(h - 1, _.length)), w < 0 && (w += _.length), w = Math.max(0, Math.min(w, _.length)), d = _.slice(h, w).join(`
`), F.hasAttribute("data-start") || F.setAttribute("data-start", String(h + 1));
            }
            R.textContent = d, t.highlightElement(R);
          },
          function(d) {
            F.setAttribute(c, b), R.textContent = d;
          }
        );
      }
      ...
      highlight: function(F) {
        for (var R = (F || document).querySelectorAll(g), i = 0, a; a = R[i++]; )
...
class be {
  ...
  buildCodePanel() {
    ...
    f.className = "copy-button", f.innerHTML = `
    ...
    `, f.title = "Copy code to clipboard";
    let c = "";
    const E = f.innerHTML, v = `
      <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
    ...
    `, b = (g) => {
      ...
      C.className = "language-cpp", C.textContent = T.source, y.appendChild(C), o.innerHTML = "", o.appendChild(y), te.highlightElement(C);
    };
    f.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(c), f.innerHTML = v, f.classList.add("copied"), setTimeout(() => {
          f.innerHTML = E, f.classList.remove("copied");
        }, 1500);
      } catch (g) {
      ...
      y.className = "tab-button", y.textContent = g.name, T === 0 && y.classList.add("active"), y.addEventListener("click", () => {
        s.querySelectorAll(".tab-button").forEach((C) => C.classList.remove("active")), y.classList.add("active"), b(T);
      }), s.appendChild(y);
    }), this.codePanel.appendChild(s), this.codePanel.appendChild(f), this.codePanel.appendChild(o), e.length > 0 && b(0);
  }
}
class Te {
  constructor(e) {
  ...
  buildTabBar() {
    ...
    let o = "";
    const f = this.copyButton.innerHTML, c = `
      <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
        <path d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"/>
      </svg>
    `;
    this.copyButton.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(o), this.copyButton.innerHTML = c, this.copyButton.classList.add("copied"), setTimeout(() => {
          this.copyButton.innerHTML = f, this.copyButton.classList.remove("copied");
        }, 1500);
      } catch (v) {
    ...
    const E = (v) => {
      ...
      if (e.querySelectorAll(".tabbed-tab-button").forEach((g, T) => {
        ...
        this.canvasContainer.style.visibility = "hidden", this.codeViewer.style.visibility = "visible", this.copyButton.style.visibility = "visible", o = b.source || "";
        const g = o.split(`
`), T = document.createElement("pre"), y = document.createElement("div");
        y.className = "tabbed-line-numbers", y.innerHTML = g.map((F, R) => R + 1).join(`
`);
        const C = document.createElement("div");
        C.className = "tabbed-code-content";
        const x = document.createElement("code");
        x.className = "language-cpp", x.textContent = o, C.appendChild(x), T.appendChild(y), T.appendChild(C), this.codeViewer.innerHTML = "", this.codeViewer.appendChild(T), te.highlightElement(x);
      }
    };
    return t.forEach((v, b) => {
      const g = document.createElement("button");
      g.className = "tabbed-tab-button", v.isShader && g.classList.add("shader-tab"), g.textContent = v.name, b === 0 && g.classList.add("active"), g.addEventListener("click", () => E(b)), e.appendChild(g);
    }), e;
  }
}
function xe(n, e) {
...
async function ye(n, e, t, r) {
  return `/demos/${n}/shadertoy.config.json` in t ? we(n, t, e, r) : _e(n, e);
}
...
async function we(n, e, t, r) {
  ...
  for (const h of E) {
    const w = o.passes[h];
    if (w)
      for (const A of ["iChannel0", "iChannel1", "iChannel2", "iChannel3"]) {
        const S = (R = w.channels) == null ? void 0 : R[A];
  ...
  for (const h of c) {
    ...
    const A = await r[w](), S = h.split("/").pop().replace(/\.[^.]+$/, "");
    v.push({
      name: S,
      source: A,
      filter: "linear",
  ...
  for (const h of E) {
    ...
    }, S = w.source || A[h], P = `/demos/${n}/${S}`;
    if (!(P in t))
      throw new Error(`Missing shader file: ${P}`);
    const I = await t[P](), X = [
      H((i = w.channels) == null ? void 0 : i.iChannel0, b),
      H((a = w.channels) == null ? void 0 : a.iChannel1, b),
      H((u = w.channels) == null ? void 0 : u.iChannel2, b),
      H((l = w.channels) == null ? void 0 : l.iChannel3, b)
    ];
    g[h] = {
      name: h,
      glslSource: I,
  ...
  const T = ((d = o.meta) == null ? void 0 : d.title) || n.split("-").map((h) => h.charAt(0).toUpperCase() + h.slice(1)).join(" "), y = ((m = o.meta) == null ? void 0 : m.author) || null, C = ((_ = o.meta) == null ? void 0 : _.description) || null, x = o.layout || "tabbed", F = o.controls ?? !0;
  return {
    root: `/demos/${n}`,
    meta: { title: T, author: y, description: C },
    layout: x,
    controls: F,
    commonSource: f,
    passes: g,
...
const Fe = "course/day3/hook-animated-tiling";
async function Ce() {
  return ye(Fe, /* @__PURE__ */ Object.assign({
    "/demos/course/day3/hook-animated-tiling/image.glsl": () => Promise.resolve().then(() => Ae).then((r) => r.default)
...
async function Le(n) {
  ...
  const t = await Ce(), r = xe(t.layout, {
    container: e,
    project: t
  }), s = new ge({
