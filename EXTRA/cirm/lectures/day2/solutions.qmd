...
#### Part C: Basic Distance Estimation (Static)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        // Update dz BEFORE z (uses current z value)
...
This produces a bright glow near the Mandelbrot boundary, fading to black farther away. The interior stays black.

#### Part D: Animated Zoom with Distance Estimation
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (i < 256) {
        ...
        if (dr > 0.0001) {
            ...
            // Scale distance by zoom for constant visual thickness
...
The key addition: multiply `dist` by `zoom` before computing the glow. Without this, the glow becomes invisibly thin at high magnification.

#### Parts E–F: Full Implementation with Smooth Coloring
...
The dual number approach packs value and derivative into a single `vec4`, making the iteration cleaner: just `z = dcAdd(dcSqr(z), c)`. The derivative propagates automatically through the arithmetic.
