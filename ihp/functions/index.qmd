...
Visualizing a function is the prototypical pullback problem.
Given $f\colon \mathbb{R}^n \to \mathbb{R}$ (or $\mathbb{C} \to \mathbb{C}$, or $\mathbb{R}^3 \to \mathbb{R}$), a shader can show it by evaluating at every pixel and mapping the result to a color.
No meshes, no marching cubes, no parametrizations — the pixel knows its coordinates, calls $f$, and is done.

We build shaders for three flavors of this:
...
The most basic thing you might want to visualize in 2D is a function $f\colon \mathbb{R}^2 \to \mathbb{R}$.
...
{{< shader layout="tabbed" level-sets-gradient >}}
...
{{< shader layout="tabbed" level-sets-log >}}
...
{{< shader layout="tabbed" level-sets-stripes >}}
...
The graph lives in $\mathbb{C}^2 \cong \mathbb{R}^4$, so we can't just plot it.

The solution is to design a coloring of the plane itself — a pattern that encodes position as color — and then *pull it back* through $f$.
That is: to visualize $f$, we don't try to draw its graph.
Instead, we paint the *range* with a known pattern, and look at what that pattern looks like when dragged back to the domain.
...
{{< shader layout="tabbed" domain-coloring >}}
...
{{< shader layout="tabbed" domain-coloring-checker >}}
...
{{< shader layout="tabbed" domain-coloring-adaptive >}}
...
{{< shader layout="tabbed" domain-coloring-sphere >}}



### 3D Scalar Functions: Slice Planes

The most direct way to visualize a 3D scalar field is also the most familiar: take 2D slices and draw contour plots on them.
...
{{< shader layout="tabbed" slice-planes >}}


### 3D Scalar Functions: Implicit Surfaces

A scalar function $f\colon \mathbb{R}^3 \to \mathbb{R}$ has level sets $f^{-1}(c)$ that are (generically) surfaces.
This is the 3D analogue of the contour plots we drew earlier — but now the level sets are objects you can light, rotate, and walk around.
...
There's no general closed-form formula for this — the surface could be any shape — so we solve it numerically by **raymarching**: walk along the ray in small steps, evaluate $f$, and watch for the value to cross the target level.
...
{{< shader layout="tabbed" implicit-surface >}}
...
A function might have a simple sphere-like level set at one value but develop handles or disconnect at another.
Watching the family evolve as $c$ changes is exactly Morse theory made visible.
...
{{< shader layout="tabbed" implicit-surface-nested >}}
...
{{< shader layout="tabbed" implicit-surface-section >}}


### 3D Scalar Functions: Volumetric Rendering

Isosurfaces show *where* a function takes a particular value — they're crisp, opaque boundaries.
But a scalar field contains information everywhere, not just on surfaces.
A probability density, a temperature distribution, an electromagnetic potential — these are quantities that fill all of space, and the interesting structure is often in *how* the field varies, not just where it crosses a threshold.

Volume rendering makes the entire field visible by treating $\rho(x,y,z)$ as a density of luminous gas.
...
{{< shader layout="tabbed" volume-section >}}
