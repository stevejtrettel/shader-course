...
//  WAVE REFRACTION — Buffer A (PDE timestep)
//
//  Symplectic Euler for u_tt = c(x)^2 * laplacian(u)
//  with spatially varying speed. Click to inject pulses.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // The PDE: wave equation  u_tt = c(x)^2 * laplacian(u)  with spatially varying speed
    float spd = speed(uv);
    float newV = v + dt * spd * spd * laplacian;
